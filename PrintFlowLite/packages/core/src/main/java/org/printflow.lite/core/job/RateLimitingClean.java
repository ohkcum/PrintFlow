 ...
 *
 */
public final class RateLimitingClean extends AbstractJob {
    ...
    public void onExecute(final JobExecutionContext ctx)
        ...
        try {
            nDeleted = ServiceContext.getServiceFactory()
                    .getRateLimiterService().cleanUp();
                ...
                if (level == PubLevelEnum.INFO) {
                    if (nDeleted == 1) {
                        msg = AppLogHelper.logInfo(getClass(),
                                "RateLimitingClean.success.single");
                    } else {
                        msg = AppLogHelper.logInfo(getClass(),
                                "RateLimitingClean.success.plural", msgParm);
                    }

                } else {
                    msg = AppLogHelper.logError(getClass(),
                            "RateLimitingClean.error", msgParm);
                }

                AdminPublisher.instance().publish(PubTopicEnum.USER, level,
                        msg);
            }
