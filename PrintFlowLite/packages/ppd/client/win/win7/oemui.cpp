...
// --------------------------------------------------------------------
// Main file for OEM UI test module.
// --------------------------------------------------------------------
...
static HRESULT hrDocumentPropertyPage(DWORD dwMode, POEMCUIPPARAM pOEMUIParam)
{
    ...
    if(NULL == pOEMUIParam->pOEMOptItems)
    {
        ...
        //pOEMUIParam->cOEMOptItems = 1; // PRINTFLOWLITE
        pOEMUIParam->cOEMOptItems = 0; // PRINTFLOWLITE
...
static HRESULT hrPrinterPropertyPage(DWORD dwMode, POEMCUIPPARAM pOEMUIParam)
{
    ...
    else
    {
        ...
        pOEMUIParam->pOEMOptItems[0].pOptType->Type = TVOT_UDARROW;
        pOEMUIParam->pOEMOptItems[0].pOptType->pOptParam[1].IconID = 1;//0;
        pOEMUIParam->pOEMOptItems[0].pOptType->pOptParam[1].lParam = 8; //100;
#endif		
    }
...
LONG APIENTRY OEMPrinterUICallBack(PCPSUICBPARAM pCallbackParam, POEMCUIPPARAM pOEMUIParam)
{
    ...
    VERBOSE("OEMPrinterUICallBack() entry.\r\n");
...
//
#pragma warning(push)
#pragma warning( disable : 6262 ) // 2 MAX_PATH strings exceeds the default prefast stack size warning threshold by a few bytes.
INT_PTR CALLBACK DevicePropPageProc(HWND hDlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	// PRINTFLOWLITE: This is a work-around to get the ShellExecute() compiled without warnings (errors).
	if (uiMsg == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && LOWORD(wParam) == IDC_CALIBRATE) {
		ShellExecute(NULL, TEXT("open"), TEXT("https://www.printflowlite.local"), NULL, NULL, SW_SHOWNORMAL);
		return FALSE;
	}
                ...
                case BN_CLICKED:
                    switch(LOWORD(wParam))
                    {
                        case IDC_CALIBRATE:
                            ...
                            // since we don't actually calibrate anything.
                            {			
								// PRINTFLOWLITE: Code below MUST be there to prevent compile errors.
...
LONG APIENTRY OEMDocUICallBack(PCPSUICBPARAM pCallbackParam, POEMCUIPPARAM pOEMUIParam)
{
    ...
    VERBOSE("OEMDocUICallBack() entry\r\n");
...
LONG APIENTRY OEMDocUICallBack2(PCPSUICBPARAM pCallbackParam)
{
    ...
    VERBOSE("OEMDocUICallBack2() entry.\r\n");
...
static PTSTR GetStringResource(HANDLE hHeap, HANDLE hModule, UINT uResource)
{
    ...
    VERBOSE("GetStringResource entered.\r\n");
