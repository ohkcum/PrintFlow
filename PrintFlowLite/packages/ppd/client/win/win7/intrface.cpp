...
// --------------------------------------------------------------------
// Implementation of interface for PScript5 and Unidrv UI plug-ins.
// --------------------------------------------------------------------
            ...
            DWORD       *dwResult)
{
    ...
    // UNREFERENCED_PARAMETER(wCapability); // PrintFlowLite
    UNREFERENCED_PARAMETER(pOutput); // PrintFlowLite
    UNREFERENCED_PARAMETER(pPublicDM);
    UNREFERENCED_PARAMETER(pOEMDM);
    UNREFERENCED_PARAMETER(dwOld);
    // UNREFERENCED_PARAMETER(dwResult); // PrintFlowLite

	// PrintFlowLite: start
	
	if (wCapability == DC_NUP) {
		if (pOutput == NULL) {
			*dwResult = 0;
		} else {
			// ???
	...
	if (wCapability == DC_COLLATE) {
		*dwResult = 0;
		return S_OK;
	}
	
	if (wCapability == DC_COPIES) {
		*dwResult = 1;
		return S_OK;
	}
	
	if (wCapability == DC_DUPLEX) {
		*dwResult = 0;
		return S_OK;
	}
