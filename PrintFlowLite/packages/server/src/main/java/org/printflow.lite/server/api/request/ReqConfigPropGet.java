...
public final class ReqConfigPropGet extends ApiRequestMixin {
    ...
    protected void onRequest(final String requestingUser, final User lockedUser)
        ...
        final boolean isUserEncrypted = cm.isUserEncrypted(key);
        HtmlButtonEnum saveButton = HtmlButtonEnum.OK;
        ...
        if (isUserEncrypted) {
            dtoRsp.setValue("");
            if (!value.isEmpty()) {
                saveButton = HtmlButtonEnum.RESET;
            }
        } else {
            dtoRsp.setValue(value);
        }
        dtoRsp.setSaveButtonText(saveButton.uiText(getLocale()));

        dtoRsp.setMultiline(Boolean.valueOf(cm.isConfigMultiline(key)));
        dtoRsp.setPassword(Boolean.valueOf(isUserEncrypted));
