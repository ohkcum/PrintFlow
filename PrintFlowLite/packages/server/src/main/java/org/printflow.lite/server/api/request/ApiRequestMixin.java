...
import org.printflow.lite.ext.notification.JobTicketCancelEvent;
import org.printflow.lite.ext.notification.JobTicketCloseEvent;
import org.printflow.lite.ext.notification.JobTicketEvent;
import org.printflow.lite.ext.notification.NotificationListener;
...
import org.printflow.lite.server.session.InboxContextSession;
...
public abstract class ApiRequestMixin implements ApiRequestHandler {
    ...
    private static final String RPFL_KEY_DTO = "dto";
    private static final String RPFL_KEY_CODE = "code";
    private static final String RPFL_KEY_RESULT = "result";
    private static final String RPFL_KEY_MSG = "msg";
    private static final String RPFL_KEY_TXT = "txt";
    ...
    /** */
    protected static final AccountVoucherService ACCOUNT_VOUCHER_SERVICE =
            ServiceContext.getServiceFactory().getAccountVoucherService();

    /** */
    protected static final AppLogService APPLOG_SERVICE =
    ...
    /**
     * @return result code or {@code null} if not present.
     */
    private Map<?, ?> getApiResult() {
        final Object obj = this.getResponseMap().get(RPFL_KEY_RESULT);
        if (obj != null && obj instanceof Map<?, ?>) {
            return (Map<?, ?>) this.getResponseMap().get(RPFL_KEY_RESULT);
        }
        return null;
    }
     ...
     * @return result code or {@code null} if not present.
     */
    protected ApiResultCodeEnum getApiResultCode() {
        final Map<?, ?> result = this.getApiResult();
        if (result != null) {
            final Object obj = result.get(RPFL_KEY_CODE);
            if (obj != null) {
                return ApiResultCodeEnum.valueOf(obj.toString());
            }
        }
        return null;
    }
    ...
    /**
     * @return result text or {@code null} if not present.
     */
    protected String getApiResultText() {
        final Map<?, ?> result = this.getApiResult();
        if (result != null) {
            final Object obj = result.get(RPFL_KEY_TXT);
            if (obj != null) {
                return obj.toString();
            }
        }
    ...
    protected final void setResponse(final AbstractDto response)
            throws IOException {
        this.getResponseMap().put(RPFL_KEY_DTO, response.asMap());
    }
    ...
    public static Map<String, Object> createApiResult(
        ...
        out.put(RPFL_KEY_RESULT, result);

        result.put(RPFL_KEY_CODE, code.getValue());

        if (msg != null) {
            result.put(RPFL_KEY_MSG, msg);
        }

        if (txt != null) {
            result.put(RPFL_KEY_TXT, txt);
        }
        return out;
    }
     ...
     */
    protected final void setApiResult(final ApiResultCodeEnum code,
            final ReqMessageEnum msgKey) {
        this.setApiResult(code, msgKey.key());
    }
