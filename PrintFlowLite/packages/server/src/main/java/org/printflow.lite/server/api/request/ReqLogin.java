...
import org.printflow.lite.core.config.SetupNeededEnum;
...
import org.printflow.lite.core.services.DeviceService.DeviceAttrLookup;
...
import org.printflow.lite.server.auth.WebAppUserAuthManager;
import org.printflow.lite.server.cometd.UserEventService;
...
public final class ReqLogin extends ApiRequestMixin
     ...
     * <li>See
     * {@link #reqLoginNew(org.printflow.lite.core.services.helpers.UserAuthModeEnum, String, String, String, boolean)}
    ...
    private void reqLogin(final UserAuthModeEnum authMode, final String authId,
        ...
        if (authMode == null) {
            ...
            if (userDb == null) {
                ...
                this.onLoginFailed(webAppType, null);
    ...
    private void reqLoginNew(final SpSession session,
        ...
        if (isInternalAdmin) {
            ...
            if (webAppType != WebAppTypeEnum.ADMIN
                ...
                this.onLoginFailed(webAppType, "msg-login-denied",
                        webAppType.getUiText(), UserAuth.getUiText(authMode),
                        authId, remoteAddr);
                return;
            }
            ...
            if (!isAuthenticated) {
                this.onLoginFailed(webAppType, "msg-login-invalid-password",
            ...
            if (authMode == UserAuthModeEnum.YUBIKEY) {
                ...
                try {
                    ...
                    this.onLoginFailed(webAppType,
                            "msg-login-yubikey-connection-error",
                 ...
                 */
                if (userDb == null) {
                    this.onLoginFailed(webAppType, "msg-login-failed",
                            webAppType.getUiText(), authId, remoteAddr);
                    return;
                }
                uid = userDb.getUserId();
                 ...
                 */
                if (userDb == null) {
                    this.onLoginFailed(webAppType, "msg-login-invalid-number",
                            webAppType.getUiText(), authId, remoteAddr);
                    return;
                }
                uid = userDb.getUserId();
            ...
            if (userDb == null) {
                ...
                if (webAppType != WebAppTypeEnum.USER) {
                    this.onLoginFailed(webAppType, "msg-login-user-not-present",
                            webAppType.getUiText(),
                ...
                /*
                 * INVARIANT : no login while registration in progress.
                 */
                final RegistrationStatusEnum userRegStatus = USER_SERVICE
                        .getUserRegistrationStatus(userDb, new Date());
                if (userRegStatus != RegistrationStatusEnum.MISSING) {
                    this.onLoginFailed(webAppType, "msg-login-failed",
                ...
                if (webAppType == WebAppTypeEnum.ADMIN && !userDb.getAdmin()) {
                    this.onLoginFailed(webAppType, "msg-login-no-admin-rights",
                            webAppType.getUiText(),
                 ...
                 */
                if (!userDb.getPerson()) {
                    this.onLoginFailed(webAppType, "msg-login-no-person",
                            webAppType.getUiText(),
                            UserAuth.getUiText(authMode), userDb.getUserId(),
                            remoteAddr);
                    return;
                ...
                if (USER_SERVICE.isUserFullyDisabled(userDb, onDate)) {
                    this.onLoginFailed(webAppType, "msg-login-disabled",
                            webAppType.getUiText(),
                            UserAuth.getUiText(authMode), userDb.getUserId(),
                            remoteAddr);
                    return;
            ...
            if (authMode == UserAuthModeEnum.YUBIKEY
                ...
                if (!isAuthenticated) {
                     ...
                     */
                    this.onLoginFailed(webAppType, "msg-login-invalid-pin",
                            webAppType.getUiText(), authId, remoteAddr);
                    return;
                }
            }
     ...
     *         {@link WebAppTypeEnum#ADMIN} and reserved user id "admin".
     * @throws SpException
     *             If MailPrint Ticketing not configured or user is not a Mail
     *             Ticket Operator.
     */
    private User resolveDocLogUser(final WebAppTypeEnum webAppType,
        ...
        if (webAppType == WebAppTypeEnum.MAILTICKETS) {
            final String operator = ConfigManager.getMailPrintTicketOperator();
            if (operator == null) {
                throw new SpException("MailPrint Ticketing not configured "
                        + "or user is not a Mail Ticket Operator.");
            }
            docLogUser = USER_DAO.findActiveUserByUserId(operator);
        } else {
    ...
    @Override
    public void onRateLimited(final RateLimiterService.IEvent event,
            final long waitMsec) {
        AdminPublisher.publish(event, PubTopicEnum.USER, waitMsec);
    }
     ...
     */
    private void onRateLimiting(final WebAppTypeEnum webAppType)
            throws EndlessWaitException {

        if (ConfigManager.isUserAuthRateLimitingEnabled()) {
            RATE_LIMITER_SERVICE
                    .consumeOrWaitForEvent(LimitEnum.USER_AUTH_FAILURE_BY_IP,
                            new RateLimiterService.IPEvent(this.getClientIP(),
                                    webAppType.getUiText().concat(" Login")),
                            this);
        }
    }
     ...
     */
    private void onLoginFailed(final WebAppTypeEnum webAppType,
        ...
        try {
            this.onRateLimiting(webAppType);
        } catch (EndlessWaitException e) {
            this.setApiResultText(ApiResultCodeEnum.UNAVAILABLE,
