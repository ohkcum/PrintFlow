...
public class DocLogItemPanel extends Panel {
    ...
    /** */
    private static final String WID_BTN_PRINT_OUT_RESOLVE_CANCELED =
            "btn-printout-resolve-canceled";
    /** */
    private static final String WID_BTN_PRINT_OUT_RESOLVE_COMPLETED =
            "btn-printout-resolve-completed";
    ...
    /** */
    private static final String WID_TOOLTIP_TITLE = "tooltip-title";
    ...
    private void populateExtSupplier(final MarkupHelper helper,
        ...
        final ReservedIppQueueEnum reservedQueue =
                obj.getPrintInReservedQueue();
        final boolean skip;

        if (obj.isExtSupplierPresent()) {
            skip = obj.getExtSupplier() == ExternalSupplierEnum.WEB_SERVICE
                    && reservedQueue != null
     ...
     * @param obj
     */
    private void populatePrintInPDF(final MarkupHelper helper,
            final DocLogItem obj) {

        final PdfInfoDto dto = DOCLOG_SERVICE.getHttpPrintInPdfInfo(
                DocLogProtocolEnum.asEnum(obj.getDeliveryProtocol()),
                obj.getExtData());

        if (dto == null) {
            helper.discloseLabel(WID_TOOLTIP_TITLE);
            return;
        }

        final TooltipPanel tooltip = new TooltipPanel(WID_TOOLTIP_TITLE);

        final StringBuilder html = new StringBuilder();

        html.append("<table data-role=\"table\" data-mode=\"reflow\" "
                + "class=\"ui-responsive table-stroke "
                + MarkupHelper.CSS_UI_MINI + "\">");
        html.append("<thead><tr>");
        html.append("<th data-priority=\"1\" />");
        html.append("<th data-priority=\"2\" />");
        html.append("<th data-priority=\"2\" />");
        html.append("</tr></thead>");

        html.append("<tbody>");
        for (Entry<String, String> entry : dto.createPropertyMap(getLocale())
                .entrySet()) {
            html.append("<tr>");
    ...
    private void populateDocOutPrintTicket(final MarkupHelper helper,
        ...
        if (obj.getJobState() != null) {
            ...
            if (obj.getCompletedDate() != null) {
                ...
                if (DateUtils.isSameDay(obj.getCreatedDate(),
                        obj.getCompletedDate())) {
                    timeTxt = localizedShortTime(obj.getCompletedDate());
                } else {
                    timeTxt = localizedShortDateTime(obj.getCompletedDate());
                }
                mapVisible.put("job-completed-date", timeTxt);
            }
        }
    }
    ...
    public void populate(final IModel<DocLogItem> model) {
        ...
        this.populatePrintInPDF(helper, obj);
        ...
        countButtons +=
                this.populateResolvePrintOutJobsState(webAppType, helper, obj);
        ...
        if (obj.getIppOptionMap() != null
            ...
            if (obj.getIppOptionMap()
                    .getPrintScaling() == PrintScalingEnum.AUTO) {
                ippValue = "(" + PrintScalingEnum.AUTO.getIppValue() + ")";
            } else {
                ippValue = "";
            }
            mapVisible.put("scaled", String.format("%s %s",
                    AdjectiveEnum.SCALED.uiText(locale), ippValue));
        }
    ...
    private int populateRefundReverse(final WebAppTypeEnum webAppType,
        ...
        if (label != null) {
            ...
            MarkupHelper.modifyLabelAttr(label, MarkupHelper.ATTR_DATA_PrintFlowLite,
                    obj.getDocLogId().toString());
     ...
     * @return number of buttons added.
     */
    private int populateResolvePrintOutJobsState(
            final WebAppTypeEnum webAppType, final MarkupHelper helper,
            final DocLogItem obj) {

        int countButtons = 0;

        final boolean isPrintoutResolveBtn =
                obj.getDocType() == DocLogDao.Type.PRINT
                        && webAppType == WebAppTypeEnum.ADMIN
                        && ConfigManager.instance().isConfigValue(
                                Key.PROXY_PRINT_JOB_STATE_EDIT_ENABLE)
                        && !obj.getJobState().isEndState()
                        && this.isAccountsEditor;

        if (isPrintoutResolveBtn) {

            this.populateResolvePrintOutJobsStateBtn(helper,
                    WID_BTN_PRINT_OUT_RESOLVE_CANCELED, HtmlButtonEnum.CANCEL,
                    IppJobStateEnum.IPP_JOB_CANCELED, obj);
            countButtons++;

            this.populateResolvePrintOutJobsStateBtn(helper,
                    WID_BTN_PRINT_OUT_RESOLVE_COMPLETED,
                    HtmlButtonEnum.COMPLETE, IppJobStateEnum.IPP_JOB_COMPLETED,
                    obj);
            countButtons++;

        } else {
            helper.discloseLabel(WID_BTN_PRINT_OUT_RESOLVE_CANCELED);
            helper.discloseLabel(WID_BTN_PRINT_OUT_RESOLVE_COMPLETED);
        }
     ...
     * @param widButton
     * @param buttonEnum
     * @param jobStateResolve
     * @param obj
     */
    private void populateResolvePrintOutJobsStateBtn(final MarkupHelper helper,
            final String widButton, final HtmlButtonEnum buttonEnum,
            final IppJobStateEnum jobStateResolve, final DocLogItem obj) {

        final Label label;

        label = helper.addButton(widButton, buttonEnum);
        ...
        MarkupHelper.modifyLabelAttr(label, MarkupHelper.ATTR_TITLE,
                NounEnum.STATUS.uiText(getLocale()).concat(" → ")
                        .concat(jobStateResolve.uiText(getLocale())));
    }
