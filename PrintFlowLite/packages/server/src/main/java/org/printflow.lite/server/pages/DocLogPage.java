...
public final class DocLogPage extends AbstractListPage {
    ...
    public DocLogPage(final PageParameters parameters) {
        ...
        ConfirmationPopupPanel pnlWlk;

        /* */
        pnlWlk = new ConfirmationPopupPanel("doclog-store-delete-popup");
        pnlWlk.populate("sp-doclog-store-delete-popup",
                CommunityDictEnum.DOC_STORE.getWord(getLocale()),
                PhraseEnum.Q_DELETE_DOCUMENT.uiText(getLocale()));
        this.add(pnlWlk);

        /* */
        pnlWlk = new ConfirmationPopupPanel("printout-status-to-canceled",
                MarkupHelper.CSS_TXT_ERROR);
        pnlWlk.populate("sp-printout-popup-jobstate-to-canceled",
                CommunityDictEnum.PROXY_PRINT.getWord(getLocale()),
                HtmlButtonEnum.CANCEL.uiText(getLocale()));
        this.add(pnlWlk);

        /* */
        pnlWlk = new ConfirmationPopupPanel("printout-status-to-completed",
                MarkupHelper.CSS_TXT_VALID);
        pnlWlk.populate("sp-printout-popup-jobstate-to-completed",
                CommunityDictEnum.PROXY_PRINT.getWord(getLocale()),
                HtmlButtonEnum.COMPLETE.uiText(getLocale()));
        this.add(pnlWlk);
    }
}
