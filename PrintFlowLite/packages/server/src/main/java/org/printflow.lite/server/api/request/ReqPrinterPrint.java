...
import org.printflow.lite.ext.papercut.job.PaperCutPrintMonitorJob;
import org.printflow.lite.server.api.JsonApiDict;
import org.printflow.lite.server.pages.user.Print;
...
public final class ReqPrinterPrint extends ApiRequestMixin {
    ...
    protected void onRequest(final String requestingUser, final User lockedUser)
        ...
        printReq.setConvertToGrayscale(!isJobTicket && PROXY_PRINT_SERVICE
        ...
        try {
            /*
             * NOTE: For JobTickets the PrintFlowLite calculated costs are leading.
             */
            if (isNonSecurePaperCutPrint && !isJobTicket) {
                ...
                final User userCheckCreditLimit =
                        isSharedAccountPrint ? null : lockedUser;

                costResult = ACCOUNTING_SERVICE.calcProxyPrintCost(
                        ServiceContext.getLocale(), currencySymbol,
                        userCheckCreditLimit, printer, costParms,
    ...
    /**
     * Validates input and applies defaults for all cases.
     *
     * @param dtoReq
     *            The user request (to which defaults are applied).
     * @return {@code true} when input is valid.
     */
    private boolean validateCommon(final DtoReq dtoReq) {

        final Integer copies = dtoReq.getCopies();
        ...
        // Apply default for missing collate option.
        if (dtoReq.getCollate() == null) {
            dtoReq.setCollate(true);
        }
    ...
    private boolean validatePdfOrientation(final DtoReq dtoReq,
        ...
        final boolean isStaple = !Objects.toString(options.get(
                IppDictJobTemplateAttr.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_STAPLE),
                IppKeyword.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_STAPLE_NONE)
                .equals(IppKeyword.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_STAPLE_NONE);

        final boolean isPunch = !Objects.toString(options
                .get(IppDictJobTemplateAttr.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_PUNCH),
                IppKeyword.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_PUNCH_NONE)
                .equals(IppKeyword.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_PUNCH_NONE);

        final boolean isBooklet = !Objects.toString(options.get(
                IppDictJobTemplateAttr.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_BOOKLET),
                IppKeyword.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_BOOKLET_NONE)
                .equals(IppKeyword.ORG_PRINTFLOWLITE_ATTR_FINISHINGS_BOOKLET_NONE);
