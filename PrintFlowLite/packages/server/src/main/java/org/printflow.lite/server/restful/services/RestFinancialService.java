...
public class RestFinancialService extends AbstractRestService
    ...
    /** */
    private static final String QUERY_PARAM_ADJUST = "adjust";
    /** */
    private static final String QUERY_PARAM_DETAILS = "details";

    /** */
    private static final String TRX_COMMENT =
            ExternalSupplierEnum.WEB_SERVICE.getUiText();
    ...
    private static Response getBalance(final AccountTypeEnum accountType,
        ...
        ServiceContext.reopen(); // Mantis #1284
    ...
    private static Response setBalance(final AccountTypeEnum accountType,
            final String accountName, final BigDecimal amount,
            final boolean adjust, final String actorUserid,
        ...
        final StringBuilder logMsg =
                new StringBuilder(String.format("%s.setBalance of %s [%s]",
        ...
        if (adjust) {
            logMsg.append(
                    String.format(" adjust with [%s]", amount.toPlainString()));
            final int userBalanceDecimals = ConfigManager.instance()
                    .getConfigInt(Key.FINANCIAL_USER_BALANCE_DECIMALS);
            balanceNew = acc.getBalance().add(amount)
                    .setScale(userBalanceDecimals, RoundingMode.DOWN);
        } else {
            balanceNew = amount;
        }
        logMsg.append(String.format(" to [%s] by [%s]",
                balanceNew.toPlainString(), actorUserid));
            ...
            if (acc == null) {
                ...
                if (trx == null) {
                    ...
                    if (StringUtils.isNotBlank(details)) {
                        trx.setExtDetails(details.trim());
                    }
                    ACCOUNT_DAO.update(acc);
                    isCommitted = Boolean.TRUE;
                    daoCtx.commit();
                    rspDto.setResult(balanceNew.toPlainString());
                }
            }
        } finally {
     ...
     *            Amount
     * @param adjust
     *            Optional. {@link true}, creates a transaction with amount.
     *            {@link false} (default) sets balance to amount by creating a
     *            transaction.
     * @param details
     *            Comment.
     *
    ...
    public Response adminSetBalance(//
            ...
            @QueryParam(QUERY_PARAM_ADJUST) //
            final Boolean adjust, //
            @QueryParam(QUERY_PARAM_DETAILS) //
            final String details) {

        final String actorUserid =
                HttpAuthenticationUtil.getBasicAuthUserPassword(authString)[0];
        return setBalance(accountType, accountName, amount,
                BooleanUtils.isTrue(adjust), actorUserid, details);
    }
