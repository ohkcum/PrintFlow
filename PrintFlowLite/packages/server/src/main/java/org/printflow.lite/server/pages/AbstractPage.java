...
public abstract class AbstractPage extends WebPage
    ...
    protected final WebAppTypeEnum
        ...
        return EnumUtils.getEnum(WebAppTypeEnum.class,
                parameters.get(WebAppParmEnum.PFL_APP.parm()).toString());
    }
    ...
    /**
     * Called after rendering of this Wicket component and all its children is
     * done and common PrintFlowLite actions are completed.
     */
    ...
    protected final void onAfterRender() {
        ...
        if (SpSession.exists()) {
            ...
            if (!session.isHumanDetected() && !session.isAuthenticated()
                 ...
                 * crawler can be simulated with `wget` of PrintFlowLite Web App.
                 */
                session.invalidate();
    ...
    protected final boolean isRestrictedToLocalLogin() {
        return this.getParmValue(this.getPageParameters(), true,
                WebAppParmEnum.PFL_LOGIN_LOCAL.parm()) != null;
    }
    ...
    protected final boolean isIntranetRequest() {
        ...
        return InetUtils.isIntranetBrowserHost(url.getHost())
                && (port.equals(ConfigManager.getServerSslPortLocal())
