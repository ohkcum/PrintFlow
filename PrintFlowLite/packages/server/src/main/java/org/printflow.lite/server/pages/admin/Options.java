...
import org.printflow.lite.server.pages.EnumRadioPanel;
...
public final class Options extends AbstractAdminPage {
    ...
    public Options(final PageParameters parameters) {
        ...
        //
        helper.encloseLabel("plugin-user-source", "PaperCut User Source",
                ConfigManager.instance()
                        .isConfigValue(Key.CUSTOM_USER_SYNC_PAPERCUT));
        ...
        labelledPasswordPanel("ldap-admin-pw", Key.AUTH_LDAP_ADMIN_PASSWORD);
        ...
        labelledCheckbox("internal-users-registration-enable",
                IConfigProp.Key.INTERNAL_USERS_REGISTRATION_ENABLE);

        this.tagInput("internal-users-registration-domain",
                IConfigProp.Key.INTERNAL_USERS_REGISTRATION_EMAIL_DOMAIN_WHITELIST);
        ...
        if (ConfigManager.hasSmtpOAuthTokenRetriever()) {
            ...
            labelledPasswordPanel(WID_SMTP_PASSWORD, Key.MAIL_SMTP_PASSWORD);
        ...
        labelledPasswordPanel("papercut-token",
        ...
        labelledPasswordPanel("papercut-db-password",
        ...
        if (ConfigManager.hasImapPrintOAuthTokenRetriever()) {
            ...
            labelledPasswordPanel(WID_IMAP_PASSWORD,
                    IConfigProp.Key.PRINT_IMAP_PASSWORD);
        ...
        labelledPasswordPanel("cliapp-auth-admin-passkey",
                IConfigProp.Key.CLIAPP_AUTH_ADMIN_PASSKEY, null);

        helper.addAppendLabelAttr("cliapp-auth-admin-passkey-confirm" + "-for",
                HtmlButtonEnum.CONFIRM.uiText(getLocale()),
                MarkupHelper.ATTR_FOR, "cliapp-auth-admin-passkey-confirm");
        this.add(PasswordPanel.createPopulate(
                "cliapp-auth-admin-passkey-confirm" + "-panel",
                "cliapp-auth-admin-passkey-confirm", null));
    ...
    private void handlePasswordPanels() {
        for (final String wid : new String[] { //
                "admin-pw-new", "admin-pw-confirm", //
                "jmx-pw-new", "jmx-pw-confirm" }) {
    ...
    /**
     * @param id
     *            {@link MarkupHelper#ATTR_ID} of {@code <input>} tag.
     * @param configKey
     */
    private void labelledPasswordPanel(final String id,
            final IConfigProp.Key configKey) {
        this.labelledPasswordPanel(id, configKey,
                PhraseEnum.KEEP_CURRENT_VALUE);
    }
     ...
     */
    private void labelledPasswordPanel(final String id,
            final IConfigProp.Key configKey, final PhraseEnum placeholder) {

        final String key = ConfigManager.instance().getConfigKey(configKey);

        final Label labelWrk =
                new Label(id + "-for", getLocalizer().getString(id, this));
        labelWrk.add(new AttributeModifier(MarkupHelper.ATTR_FOR, key));
        this.add(labelWrk);

        final Map<String, String> map = new HashMap<>();
        if (placeholder != null) {
            map.put(MarkupHelper.ATTR_PLACEHOLDER,
                    placeholder.uiText(this.getLocale()));
        }
        this.add(PasswordPanel.createPopulate(id + "-panel", key, null, map));
    }
    ...
    private void setReadOnlyAccess(final MarkupHelper helper,
        ...
        // Disable sections
        for (final String wicketId : new String[] { //
                "sect-user-source", "sect-user-creation",
                "sect-user-authentication", "sect-mail", "sect-papercut",
                "sect-mail-print", "sect-web-print", "sect-internet-print",
                "sect-proxy-print", "sect-eco-print", "sect-financial",
                "sect-backups", "sect-telegram", "sect-advanced" }) {
        ...
        // Hide Apply buttons
        for (final String wicketId : new String[] { //
                "btn-apply-auth", "btn-apply-internal-users",
                "btn-apply-user-source-group-apply", "btn-user-sync-apply",
                "btn-apply-user-create", "btn-apply-user-auth-mode-local",
                "btn-apply-smtp", "btn-apply-mail", "btn-apply-papercut",
                "btn-apply-imap", "btn-apply-webprint",
                "btn-apply-internetprint", "btn-apply-proxyprint",
                "btn-apply-proxyprint-papercut", "btn-apply-ecoprint",
                ...
                "btn-cliapp-admin-passkey-reset", "btn-admin-pw-reset",
                "btn-jmx-pw-reset", "btn-apply-locale", "btn-apply-papersize",
                "btn-apply-report-fontfamily", "btn-apply-converters",
                "btn-apply-print-in", "btn-apply-telegram",
                "btn-pagometer-reset" }) {
        ...
        helper.encloseLabel("btn-atomfeed-refresh",
                HtmlButtonEnum.REFRESH.uiText(getLocale()),
                !readonly && ConfigManager.instance()
