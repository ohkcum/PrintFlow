...
import org.printflow.lite.server.pages.JrExportFileExtButtonPanel;
...
public final class UsersBase extends AbstractAdminPage {
    ...
    /** */
    private static final String WICKET_ID_PROMPT_USER_REGISTRATION =
    ...
    public UsersBase(final PageParameters parameters) {
        ...
        if (cm.isConfigValue(Key.INTERNAL_USERS_REGISTRATION_ENABLE)) {
            helper.addLabel(WICKET_ID_PROMPT_USER_REGISTRATION,
                    NounEnum.REGISTRATION);
            helper.addLabel("user-registration-requests",
                    NounEnum.REQUEST.uiText(getLocale(), true));
        } else {
            helper.discloseLabel(WICKET_ID_PROMPT_USER_REGISTRATION);
        }
        ...
        //
        final boolean isAppReady =
                cm.getSystemStatus() != SystemStatusEnum.SETUP;
