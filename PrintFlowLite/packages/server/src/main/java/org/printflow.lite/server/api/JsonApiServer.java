 ...
 */
package org.printflow.lite.server.api;
...
import org.printflow.lite.core.cometd.CometdClientMixin;
...
import org.printflow.lite.core.dto.PrimaryKeyDto;
...
import org.printflow.lite.core.dto.VoucherBatchPrintDto;
...
import org.printflow.lite.core.inbox.PageImages.PageImage;
...
import org.printflow.lite.core.print.proxy.ProxyPrintAuthManager;
import org.printflow.lite.core.reports.JrExportFileExtEnum;
import org.printflow.lite.core.reports.JrPosDepositReceipt;
import org.printflow.lite.core.reports.JrVoucherPageDesign;
...
import org.printflow.lite.core.reports.impl.UserListReport;
import org.printflow.lite.core.reports.impl.UserPrintOutTotalsReport;
...
import org.printflow.lite.core.services.helpers.PdfOutDto;
...
import org.printflow.lite.ext.payment.PaymentGatewayPlugin;
import org.printflow.lite.ext.payment.PaymentGatewayPlugin.PaymentRequest;
...
import org.printflow.lite.server.api.request.ApiRequestHandler;
import org.printflow.lite.server.api.request.ApiRequestHelper;
...
import org.printflow.lite.server.api.request.export.ReqExportDocStorePdf;
import org.printflow.lite.server.api.request.export.ReqExportOutboxPdf;
import org.printflow.lite.server.api.request.export.ReqExportPrinterOpt;
import org.printflow.lite.server.api.request.export.ReqExportPrinterPPD;
import org.printflow.lite.server.api.request.export.ReqExportPrinterPPDExt;
import org.printflow.lite.server.api.request.export.ReqExportPrinterRawPPD;
import org.printflow.lite.server.api.request.export.ReqExportPrinterRawTransform;
import org.printflow.lite.server.api.request.export.ReqExportUserDataHistory;
import org.printflow.lite.server.cometd.AbstractEventService;
import org.printflow.lite.server.dropzone.PdfPgpDropZoneFileResource;
import org.printflow.lite.server.dropzone.PdfPgpUploadHelper;
import org.printflow.lite.server.dropzone.PdfValidateDropZoneFileResource;
import org.printflow.lite.server.dropzone.PdfValidateUploadHelper;
import org.printflow.lite.server.dropzone.WebPrintDropZoneFileResource;
import org.printflow.lite.server.dropzone.WebPrintHelper;
import org.printflow.lite.server.dto.MoneyTransferDto;
...
public final class JsonApiServer extends AbstractPage {
     ...
     * The logger.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(JsonApiServer.class);

    /** */
    private static final AccountingService ACCOUNTING_SERVICE = ServiceContext.getServiceFactory()
            .getAccountingService();

    /** */
    private static final AccountVoucherService ACCOUNT_VOUCHER_SERVICE = ServiceContext.getServiceFactory()
            .getAccountVoucherService();

    /** */
    private static final EmailService EMAIL_SERVICE = ServiceContext.getServiceFactory().getEmailService();

    /**
     *
     */
    private static final DocLogService DOC_LOG_SERVICE = ServiceContext.getServiceFactory().getDocLogService();

    /** */
    private static final InboxService INBOX_SERVICE = ServiceContext.getServiceFactory().getInboxService();

    /** */
    private static final JobTicketService JOBTICKET_SERVICE = ServiceContext.getServiceFactory().getJobTicketService();

    /** */
    private static final ProxyPrintService PROXY_PRINT_SERVICE = ServiceContext.getServiceFactory()
            .getProxyPrintService();

    /** */
    private static final UserService USER_SERVICE = ServiceContext.getServiceFactory().getUserService();
    ...
    public JsonApiServer(final PageParameters parameters) {
        ...
        final IRequestParameters reqParms = requestCycle.getRequest().getPostParameters();

        /*
         * Get the request id + requesting user
         */
        String requestId = reqParms.getParameterValue(JsonApiDict.PARM_REQ).toString();
        ...
        final String requestingUser = getParmValue(parameters, isGetAction, JsonApiDict.PARM_USER);

        final WebAppTypeEnum requestingWebAppType = EnumUtils.getEnum(WebAppTypeEnum.class, getParmValue(parameters,
        ...
        JsonApiDict.LetterheadLock letterheadLock = JsonApiDict.LetterheadLock.NONE;
        ...
        try {
            ...
            if (isValidAndAuthorizedRequest) {
                ...
                if (!isInternalAdmin
                    ...
                    User userTmp = userDao.findActiveUserByUserId(requestingUser);
                ...
                switch (requestId) {
                    ...
                    case JsonApiDict.REQ_PRINTER_PPDE_DOWNLOAD:
                        // no break intended
                    case JsonApiDict.REQ_PRINTER_RAWPRINT_PPD_DOWNLOAD:
                        // no break intended
                    case JsonApiDict.REQ_PRINTER_RAWPRINT_TRANSFORM_DOWNLOAD:
                        // no break intended
                ...
                if (requestId != null
                    ...
                    final String urlPath = WebApp.getMountPath(getSessionWebAppType());

                    final StringBuilder html = new StringBuilder();

                    html.append("<h2 style='color: red;'>")
                            .append(CommunityDictEnum.PrintFlowLite.getWord())
    ...
    private void handleExportFile(final String request,
        ...
        final String baseName = "export-" + UUID.randomUUID().toString() + ".tmp";
        ...
        try {
            ...
            switch (request) {
                ...
                case JsonApiDict.REQ_PDF_DOCSTORE_ARCHIVE:
                    requestHandler = new ReqExportDocStorePdf(DocStoreTypeEnum.ARCHIVE)
                ...
                case JsonApiDict.REQ_PDF_DOCSTORE_JOURNAL:
                    requestHandler = new ReqExportDocStorePdf(DocStoreTypeEnum.JOURNAL)
                            .export(getSessionWebAppType(),
                                    getRequestCycle(), parameters,
                                    isGetAction, requestingUser, null);
                    break;
                ...
                case JsonApiDict.REQ_REPORT_USER:
                    ...
                    requestHandler = exportReport(tempExportFile, EnumUtils.getEnum(
                            JrExportFileExtEnum.class,
                            Objects.toString(StringUtils.stripToNull(parameters
                                    .get(JsonApiDict.PARM_REQ_PARM).toString()),
                ...
                case JsonApiDict.REQ_PRINTER_RAWPRINT_PPD_DOWNLOAD:
                    requestHandler = new ReqExportPrinterRawPPD(
                            parameters.get(JsonApiDict.PARM_REQ_SUB).toLongObject())
                            .export(getSessionWebAppType(),
                                    getRequestCycle(), parameters,
                                    isGetAction, requestingUser, null);
                ...
                case JsonApiDict.REQ_PRINTER_RAWPRINT_TRANSFORM_DOWNLOAD:
                    requestHandler = new ReqExportPrinterRawTransform(
                            parameters.get(JsonApiDict.PARM_REQ_SUB).toLongObject())
                            .export(getSessionWebAppType(),
                                    getRequestCycle(), parameters,
                                    isGetAction, requestingUser, null);
     ...
     *                            The database primary key of the
     *                            {@link AccountTrx}.
     * @param requestingUser
     *                            The requesting userid.
     * @param requestingUserAdmin
     *                            {@code true} if requesting user is an
     *                            administrator.
     ...
     *                       {@code true} if this is a request from an
     *                       administrator.
     * @throws JRException
    ...
    private IRequestHandler exportPosPurchaseDocument(
        ...
        final PosDepositReceiptDto receipt = this.createPosPurchaseDto(accountTrxType, tempFile,
                accountTrxDbId, requestingUser, requestingUserAdmin);

        final ResourceStreamRequestHandler handler = new DownloadRequestHandler(tempFile);
    ...
    private IRequestHandler exportVoucherBatchPrint(final File tempFile,
        ...
        final VoucherBatchPrintDto dto = AbstractDto.create(VoucherBatchPrintDto.class, jsonDto);

        final InternalFontFamilyEnum font = ConfigManager
                .getConfigFontFamily(Key.FINANCIAL_VOUCHER_CARD_FONT_FAMILY);

        final JasperDesign design = dto.getDesign().createDesign(font, reportlocale);
        ...
        final String userFriendlyFilename = CommunityDictEnum.PrintFlowLite.getWord()
    ...
    private IRequestHandler exportReport(final File tempExportFile,
        ...
        final ResourceStreamRequestHandler handler = new DownloadRequestHandler(tempExportFile);
    ...
    private IRequestHandler exportPapercutDelegatorCost(
        ...
        final DelegatedPrintPeriodDto dto = AbstractDto.create(DelegatedPrintPeriodDto.class, jsonData);
        ...
        final DownloadRequestHandler handler = new DownloadRequestHandler(tempCsvFile);
    ...
    private IRequestHandler handleExportSafePages(final User lockedUser,
        ...
        final String jsonDto = getParmValue(parameters, isGetAction,
                PdfOutDto.class.getSimpleName());

        final PdfOutDto pdfOutDto = PdfOutDto.create(jsonDto);

        final boolean removeGraphics = BooleanUtils.isTrue(pdfOutDto.getRemoveGraphics());
        final boolean ecoPrint = BooleanUtils.isTrue(pdfOutDto.getEcoprint());
        ...
        try {
             ...
             * (1) Generate the PDF
             */
            pdfTemp = this.generatePdfForExport(lockedUser,
                    Integer.parseInt(
                            getParmValue(parameters, isGetAction, "jobIndex")),
                    getParmValue(parameters, isGetAction, "ranges"), pdfOutDto,
                    ConfigManager.instance().getConfigEnum(
                            PdfResolutionEnum.class,
                            Key.PDF_OUT_RASTERIZE_DPI_DOWNLOAD),
                    docLog, "download");
             ...
             * Stream PDF
             */
            ResourceStreamRequestHandler handler = new PdfFileRequestHandler(lockedUser.getUserId(), pdfTemp);
    ...
    private Map<String, Object> handleRequest(final String request,
        ...
        switch (request) {
            ...
            case JsonApiDict.REQ_PAYMENT_GATEWAY_ONLINE:
                ...
                return reqPaymentGatewayOnline(Boolean.parseBoolean(
            ...
            case JsonApiDict.REQ_SEND:
                ...
                return reqSend(lockedUser,
                        ...
                        getParmValue(parameters, isGetAction, "jobIndex"),
                        getParmValue(parameters, isGetAction, "ranges"),
                        PdfOutDto.create(getParmValue(parameters, isGetAction,
                                PdfOutDto.class.getSimpleName())));
            ...
            default:
                ...
                final ApiRequestHandler apiReqHandler = API_DICTIONARY.createApiRequest(request);
     ...
     *                        The zero-based index of the vanilla job. If this index
     *                        is LT
     *                        zero the pageRangeFilter refers to the integrated
     *                        document.
     * @param pageRangeFilter
     *                        The page range filter. For example: '1,2,5-6'. The
     *                        page
     *                        numbers in page range filter refer to one-based page
     *                        numbers
     *                        of the integrated {@link InboxInfoDto} document OR for
     *                        a
     *                        single vanilla job. When {@code null}, then the full
     *                        page
     *                        range is applied.
     * @param pdfOutDto
     ...
     *                                         When {@link EcoPrintPdfTask} objects
     *                                         needed for this PDF are
     *                                         pending.
     */
    private File generatePdfForExport(final User user,
            final int vanillaJobIndex, final String pageRangeFilter,
            final PdfOutDto pdfOutDto, final PdfResolutionEnum rasterizeDpi,
        ...
        final String pdfFile = OutputProducer.createUniqueTempPdfName(user, purpose);
        ...
        pdfRequest.setRemoveGraphics(
                BooleanUtils.isTrue(pdfOutDto.getRemoveGraphics()));
        pdfRequest.setApplyPdfProps(true);
        pdfRequest.setApplyLetterhead(true);
        pdfRequest.setForPrinting(false);
        pdfRequest
                .setEcoPdfShadow(BooleanUtils.isTrue(pdfOutDto.getEcoprint()));
        pdfRequest.setGrayscale(BooleanUtils.isTrue(pdfOutDto.getGrayscale()));
        pdfRequest.setRasterized(BooleanUtils.isTrue(pdfOutDto.getRasterize()));
        pdfRequest.setRasterizedResolution(rasterizeDpi);

        pdfRequest.setLinks(BooleanUtils.isTrue(pdfOutDto.getLinks()));
    ...
    private String calcDocumentPageRangeFilter(final User user,
        ...
        if (vanillaJobIndex < 0) {
             ...
             */
            final InboxInfoDto jobInfo = INBOX_SERVICE.readInboxInfo(user.getUserId());
            ...
            if (!isShutdown) {
                if (exception instanceof LockAcquisitionException
                    ...
                    LOGGER.error(String.format("[%s][%s][%s]: %s", requestId,
                            Objects.toString(requestingUser, "-"),
    ...
    private Map<String, Object> reqSend(final User lockedUser,
            ...
            final PdfOutDto pdfOutDto) throws LetterheadNotFoundException,
            IOException, MessagingException, InterruptedException,
            CircuitBreakerException, ParseException, PGPBaseException {

        final Map<String, Object> userData = new HashMap<String, Object>();

        if (BooleanUtils.isTrue(pdfOutDto.getRemoveGraphics())
                && BooleanUtils.isTrue(pdfOutDto.getEcoprint())) {
        ...
        try {
             ...
             */
            fileAttach = this.generatePdfForExport(lockedUser,
                    Integer.parseInt(jobIndex), ranges, pdfOutDto,
                    ConfigManager.instance().getConfigEnum(
                            PdfResolutionEnum.class,
                            Key.PDF_OUT_RASTERIZE_DPI_EMAIL),
                    docLog, "email");
            /*
            ...
            if (fileAttach.length() > maxFileBytes) {
                ...
                final BigDecimal maxFileMb = new BigDecimal((double) maxFileKb / 1024);
    ...
    private Map<String, Object> reqPrinterDetail(final String user,
        ...
        final JsonPrinterDetail jsonPrinter = PROXY_PRINT_SERVICE.getPrinterDetailUserCopy(
    ...
    private Map<String, Object> reqPrintFastRenew(final String requestingUser) {
        ...
        final InboxInfoDto dto = INBOX_SERVICE.touchLastPreviewTime(requestingUser);

        final SimpleDateFormat timeFormat = new SimpleDateFormat("HH:mm:ss");

        final String expiry = timeFormat.format(new Date(dto.getLastPreviewTime().longValue()
     ...
     */
    private Map<String, Object> reqPaymentGatewayOnline(final boolean online)
            throws PaymentGatewayException {
        ...
        final PaymentGatewayPlugin plugin = manager.getExternalPaymentGateway();
        ...
        if (plugin != null) {
            ...
            final String systemMsg = AppLogHelper.logInfo(getClass(), key, plugin.getName());
    ...
    private Map<String, Object> reqUserPasswordReset(
        ...
        final String encryptedPw = USER_SERVICE.encryptUserPassword(iuser, password);
    ...
    private Map<String, Object> reqPrinterSetMediaCost(final String jsonCost) {
        ...
        final ProxyPrinterCostDto dto = JsonAbstractBase.create(ProxyPrinterCostDto.class, jsonCost);
         ...
         *
         */
        final AbstractJsonRpcMethodResponse rpcResponse = PROXY_PRINT_SERVICE.setProxyPrinterCostMedia(jpaPrinter, dto);
    ...
    private boolean validateUserPin(final Map<String, Object> userData,
        ...
        final int lengthMin = ConfigManager.instance().getConfigInt(Key.USER_PIN_LENGTH_MIN);

        final int lengthMax = ConfigManager.instance().getConfigInt(Key.USER_PIN_LENGTH_MAX);
    ...
    private static Map<String, Object> apiResultFromBasicRpcResponse(
        ...
        if (rpcResponse.isResult()) {
            ...
            final ErrorDataBasic error = rpcResponse.asError().getError().data(ErrorDataBasic.class);
    ...
    private Map<String, Object> reqUserMoneyTransfer(
        ...
        final MoneyTransferDto dto = JsonAbstractBase.create(MoneyTransferDto.class, jsonDto);

        /*
         * INVARIANT: Amount MUST be valid.
         */
        final String plainAmount = dto.getAmountMain() + "." + dto.getAmountCents();
    ...
    private Map<String, Object> reqPosDeposit(final String requestingUser,
        ...
        final PosDepositDto dto = JsonAbstractBase.create(PosDepositDto.class, jsonDto);

        final AbstractJsonRpcMethodResponse rpcResponse = ACCOUNTING_SERVICE.depositFunds(dto);
    ...
    private Map<String, Object> reqPosReceiptSendMail(
        ...
        final AccountTrxDao accountTrxDao = ServiceContext.getDaoContext().getAccountTrxDao();
        ...
        final UserAccountDao userAccountDao = ServiceContext.getDaoContext().getUserAccountDao();
    ...
    private Map<String, Object> reqUserSourceGroups(final String user) {
        ...
        final SortedSet<CommonUserGroup> sset = ConfigManager.instance().getUserSource().getGroups();
    ...
    private Map<String, Object> reqConstants(final WebAppTypeEnum webAppType,
        ...
        /*
         * Is OAuth the only login method?
         */
        final boolean isOAuthOnly;
        if (!webAppType.isEndUserType() || WebApp.get().getPluginManager()
                .getOAuthClientPlugins().isEmpty()) {
            isOAuthOnly = false;
        } else {
            final Set<UserAuthModeEnum> setAllowed = userAuth.getAuthModesAllowed();
            isOAuthOnly = setAllowed.isEmpty() || (setAllowed.size() == 1
                    && setAllowed.contains(UserAuthModeEnum.OAUTH));
        }
        /*
         * Set default authentication mode to "null" if OAuth is the only
         * authentication method. In this way the JavaScript client will only
         * show the OAuth button(s).
         */
        final String authModeDefault = isOAuthOnly ? null : userAuth.getAuthModeDefault().toDbValue();
        userData.put("authModeDefault", authModeDefault);
        ...
        userData.put("watchdogIntervalSecs",
                cm.getConfigInt(Key.WEBAPP_WATCHDOG_INTERVAL_SECS));
        userData.put("watchdogAllowedDelaySecs",
                cm.getConfigInt(Key.WEBAPP_WATCHDOG_ALLOWED_DELAY_SECS));
        userData.put("watchdogMessageEnable",
                cm.isConfigValue(Key.WEBAPP_WATCHDOG_MESSAGE_ENABLE));
        ...
        final int minutes = cm.getConfigInt(Key.JOBTICKET_DELIVERY_DAY_MINUTES, 0);
        ...
        if (cm.isConfigValue(Key.PROXY_PRINT_DELEGATE_ENABLE)) {
            ...
            if (cm.isConfigValue(
                ...
                userDetail = QuickSearchUserGroupMemberFilterDto.UserDetail.NAME;
            } else {
        ...
        // Web Print
        final boolean isWebPrintEnabled = WebPrintHelper.isWebPrintEnabled(this.getClientIP());
        ...
        if (isWebPrintEnabled) {
            ...
            userData.put("webPrintMaxBytes",
                    WebPrintHelper.instance().getMaxUploadSize().bytes());
            userData.put("webPrintUploadUrl",
                    WebApp.MOUNT_PATH_UPLOAD_WEBPRINT);
            userData.put("webPrintUploadFileParm",
                    WebPrintDropZoneFileResource.getUploadFileParm());
            userData.put("webPrintUploadFontParm",
                    WebPrintDropZoneFileResource.getUploadFontParm());
            userData.put("webPrintFileExt",
                    WebPrintHelper.instance().getSupportedFileExtensions(true));
        }

        if (ConfigManager.isPdfPgpEnabled()) {
            userData.put("pdfpgpMaxBytes",
                    PdfPgpUploadHelper.instance().getMaxUploadSize().bytes());
            userData.put("pdfpgpUploadUrl",
                    WebApp.MOUNT_PATH_UPLOAD_PDF_VERIFY);
            userData.put("pdfpgpUploadFileParm",
                    PdfPgpDropZoneFileResource.getUploadFileParm());
            userData.put("pdfpgpFileExt", PdfPgpUploadHelper.instance()
                    .getSupportedFileExtensions(true));
        }

        if (cm.isConfigValue(Key.WEBAPP_PDF_VALIDATE_ENABLE)) {
            userData.put("pdfValidateMaxBytes", PdfValidateUploadHelper
                    .instance().getMaxUploadSize().bytes());
            userData.put("pdfValidateUploadUrl",
                    WebApp.MOUNT_PATH_UPLOAD_PDF_VALIDATE);
            userData.put("pdfValidateUploadFileParm",
                    PdfValidateDropZoneFileResource.getUploadFileParm());
            userData.put("pdfValidateFileExt", PdfValidateUploadHelper
                    .instance().getSupportedFileExtensions(true));
        }
        ...
        if (webAppType == WebAppTypeEnum.POS) {
            ...
            final String urlPath = ServerUrlPath.CUSTOM_WEB.concat("/");
     ...
     *
     * @param language
     *                 The selected language for the Web App. This value is also
     *                 used
    ...
    private Map<String, Object> reqLanguage(final String language,
        ...
        try {
            ...
            if (StringUtils.isNotBlank(country)) {
                 ...
                 * language in the PrintFlowLite Web App.
                 */
        ...
        /*
         * Load i18n strings. If the API package i18n XML bundle is missing
         * (e.g., not bundled in this build), proceed with an empty bundle so
         * the language switch still succeeds.
         */
        try {
            final ResourceBundle rcBundle = Messages.loadXmlResource(getClass(), "i18n", locale);
            ...
            LOGGER.warn("i18n resource bundle missing for locale [{}]: {}",
                    locale, e.getMessage());
        }
