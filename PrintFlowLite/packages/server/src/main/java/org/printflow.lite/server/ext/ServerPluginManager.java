...
import org.printflow.lite.core.ipp.routing.IppRoutingContext;
...
import org.printflow.lite.ext.ServerPluginContext;
...
import org.printflow.lite.ext.notification.NotificationEventProgressImpl;
import org.printflow.lite.ext.notification.NotificationListener;
import org.printflow.lite.ext.notification.NotificationPlugin;
...
import org.printflow.lite.ext.print.IppRoutingPlugin;
import org.printflow.lite.ext.print.OAuthImapPlugin;
import org.printflow.lite.ext.rest.RestClient;
import org.printflow.lite.ext.smtp.OAuthSmtpPlugin;
...
import org.printflow.lite.server.callback.CallbackServlet;
...
public final class ServerPluginManager implements PaymentGatewayListener,
     ...
     * Property key prefix.
     */
    private static final String PROP_KEY_PLUGIN_PFX = "PrintFlowLite.plugin.";
    ...
    private boolean loadPlugin(final InputStream istr, final String istrName)
        ...
        try {
            ...
            if (plugin instanceof PaymentGateway) {
                ...
                final OAuthClientPlugin pluginFound =
                        mapPluginInstance.get(oauthPlugin.getInstanceId());

                if (pluginFound != null) {
                    LOGGER.warn(
                            "{} {} Instance [{}]: [{}] overwritten with [{}].",
                            pluginType, oauthPlugin.getProvider().toString(),
                            oauthPlugin.getInstanceId(), pluginFound.getId(),
                            oauthPlugin.getId());
                }
    ...
    private void validateOAuthCallbackUrl(final OAuthClientPlugin plugin)
        ...
        query.append(WebAppParmEnum.PFL_OAUTH.parm()).append('=')
                .append(plugin.getProvider().toString().toLowerCase());

        path.append(WebApp.MOUNT_PATH_WEBAPP_USER).append("/")
                .append(WebAppParmEnum.PFL_OAUTH.parm()).append('/')
                .append(plugin.getProvider().toString().toLowerCase());

        if (StringUtils.isNotBlank(plugin.getInstanceId())) {

            query.append('&').append(WebAppParmEnum.PFL_OAUTH_ID.parm())
    ...
    public static String
            getOAuthClientIconPath(final OAuthClientPlugin plugin) {
        ...
        return String.format("/%s/%s", CustomWebServlet.URL_PATH_BASE,
