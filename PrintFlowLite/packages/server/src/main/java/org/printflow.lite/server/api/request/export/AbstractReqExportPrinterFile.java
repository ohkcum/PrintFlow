/*
 * This file is part of the PrintFlowLite project <https://www.PrintFlowLite.org>.
 * Copyright (c) 2026 Datraverse B.V.
 * Author: Rijk Ravestein.
 *
 * SPDX-FileCopyrightText: (c) 2026 Datraverse B.V. <info@datraverse.com>
 * SPDX-License-Identifier: AGPL-3.0-or-later
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 *
 * For more information, please contact Datraverse B.V. at this
 * address: info@datraverse.com
 */
package org.printflow.lite.server.api.request.export;

import java.io.File;

import org.apache.commons.lang3.StringUtils;
import org.apache.wicket.request.IRequestHandler;
import org.apache.wicket.request.cycle.RequestCycle;
import org.apache.wicket.request.handler.resource.ResourceStreamRequestHandler;
import org.apache.wicket.request.mapper.parameter.PageParameters;
import org.apache.wicket.request.resource.ContentDisposition;
import org.apache.wicket.util.resource.FileResourceStream;
import org.printflow.lite.core.SpException;
import org.printflow.lite.core.config.WebAppTypeEnum;
import org.printflow.lite.core.dao.PrinterDao;
import org.printflow.lite.core.dao.enums.PrinterAttrEnum;
import org.printflow.lite.core.jpa.Printer;
import org.printflow.lite.core.services.ServiceContext;
import org.printflow.lite.core.services.helpers.PrinterAttrLookup;
import org.printflow.lite.server.webapp.WebAppHelper;

/**
 *
 * @author Rijk Ravestein
 *
 */
public abstract class AbstractReqExportPrinterFile
        extends ApiRequestExportMixin {

    /** */
    private final Long printerID;

    /**
     *
     * @param id
     *            The Database key of printer.
     */
    public AbstractReqExportPrinterFile(final Long id) {
        this.printerID = id;
    }

    /**
     * @return The enum value of the printer attribute holding the base file
     *         name.
     */
    protected abstract PrinterAttrEnum getPrinterAttrEnum();

    /**
     * @param fileBaseName
     * @return the full path file.
     */
    protected abstract File getExportFile(String fileBaseName);

    @Override
    protected final IRequestHandler onExport(final WebAppTypeEnum webAppType,
            final String requestingUser, final RequestCycle requestCycle,
            final PageParameters parameters, final boolean isGetAction,
            final File tempExportFile) {

        // tempExportFile is NOT applicable.

        final PrinterDao printerDao =
                ServiceContext.getDaoContext().getPrinterDao();

        final Printer printer = printerDao.findById(this.printerID);

        /*
         * INVARIANT: printer must exist in database.
         */
        if (printer == null) {
            throw new SpException(
                    "Printer [" + this.printerID + "] not found.");
        }

        final PrinterAttrLookup lookup = new PrinterAttrLookup(printer);
        final String fileBaseName = lookup.get(this.getPrinterAttrEnum());

        /*
         * INVARIANT: attribute must exist in database.
         */
        if (StringUtils.isBlank(fileBaseName)) {
            throw new SpException(String.format("Printer [%d]: [%s] NOT found.",
                    this.printerID, this.getPrinterAttrEnum().toString()));
        }

        final File filePPDExt = this.getExportFile(fileBaseName);

        /*
         * INVARIANT: file must exist.
         */
        if (!filePPDExt.exists()) {
            throw new SpException(String.format("Printer [%d]: [%s] NOT found.",
                    this.printerID, filePPDExt.getAbsolutePath()));
        }
        final FileResourceStream stream = new FileResourceStream(filePPDExt);

        final ResourceStreamRequestHandler handler =
                new ResourceStreamRequestHandler(stream);

        handler.setFileName(fileBaseName);
        handler.setContentDisposition(ContentDisposition.ATTACHMENT);

        WebAppHelper.setCacheDurationZero(handler);

        return handler;
    }
}
