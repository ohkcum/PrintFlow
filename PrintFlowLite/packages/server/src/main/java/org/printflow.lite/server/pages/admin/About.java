...
import org.printflow.lite.core.doc.XpsToPdf;
...
import org.printflow.lite.core.system.SystemInfo.SysctlEnum;
...
import org.printflow.lite.server.pages.PrinterDriverDownloadPanel;
...
public final class About extends AbstractAdminPage {
    ...
    public About(final PageParameters parameters) {
        ...
        add(new Label("version-date", localizedDateTime(
                org.printflow.lite.core.VersionInfo.getBuildDate())));
        ...
        builder.append(org.printflow.lite.core.VersionInfo.DB_SCHEMA_VERSION_MAJOR)
                .append(".")
                .append(org.printflow.lite.core.VersionInfo.DB_SCHEMA_VERSION_MINOR);
        add(new Label("version-db-schema", builder.toString()));

        //
        add(new Label("version-dir-layout-header",
                NounEnum.FILE.uiText(getLocale(), true)));
        builder = new StringBuilder();
        builder.append(org.printflow.lite.core.VersionInfo.DIR_LAYOUT_VERSION_MAJOR)
                .append(".")
                .append(org.printflow.lite.core.VersionInfo.DIR_LAYOUT_VERSION_MINOR);
        add(new Label("version-dir-layout", builder.toString()));
        ...
        add(new Label("sp-server-home-key",
                SystemPropertyEnum.PRINTFLOWLITE_SERVER_HOME.getKey()));
        add(new Label("sp-server-home",
                SystemPropertyEnum.PRINTFLOWLITE_SERVER_HOME.getValue()));

        add(new Label("app.dir.tmp-key", ServerPropEnum.APP_DIR_TMP.key()));
        add(new Label("app.dir.tmp", ConfigManager.getAppTmpDir()));

        add(new Label("app.dir.safepages-key",
                ServerPropEnum.APP_DIR_SAFEPAGES.key()));
        add(new Label("app.dir.safepages",
                ConfigManager.getSafePagesHomeDir()));

        add(new Label("app.dir.letterheads-key",
                ServerPropEnum.APP_DIR_LETTERHEADS.key()));
        add(new Label("app.dir.letterheads", ConfigManager.getLetterheadDir()));

        add(new Label("app.dir.doc.store.archive-key",
                ServerPropEnum.APP_DIR_DOC_STORE_ARCHIVE.key()));
        ...
        add(new Label("app.dir.doc.store.journal-key",
                ServerPropEnum.APP_DIR_DOC_STORE_JOURNAL.key()));
        add(new Label("app.dir.doc.store.journal", ConfigManager
                .getDocStoreHome(DocStoreTypeEnum.JOURNAL).toString()));

        add(new Label("webapp.custom.i18n-key",
                ServerPropEnum.WEBAPP_CUSTOM_I18N.key()));
        add(new Label("webapp.custom.i18n", ConfigManager
                .getServerPropValue(ServerPropEnum.WEBAPP_CUSTOM_I18N)));

        helper.encloseLabel("btn-i18n-cache-clear",
                HtmlButtonEnum.CLEAR.uiText(getLocale()), hasEditorAccess);

        // ---------- CUPS
        version = IPP_CLIENT_SERVICE.getCupsVersion();
        ...
        // ----------heif-convert
        colorInstalled = null;
        installed = SystemInfo.isHeifConvertInstalled();
        keyInstalled = installed ? "installed" : "not-installed";
        colorInstalled = installed ? null : MarkupHelper.CSS_TXT_WARN;

        labelWrk = new Label("version-heif-convert", localized(keyInstalled));
        if (colorInstalled != null) {
            labelWrk.add(new AttributeModifier(MarkupHelper.ATTR_CLASS,
                    colorInstalled));
        }
        ...
        final String[][] hostPackageInfo = new String[][] {
                ...
                { "version-avahi-browse", SystemInfo.getAvahiBrowseVersion() },
        ...
        helper.addLabel("cmd-avahi-browse",
                SystemInfo.Command.AVAHI_BROWSE.cmd());
        ...
        helper.addLabel("cmd-heif-convert",
                SystemInfo.Command.HEIF_CONVERT.cmd());
        ...
        case WRONG_MODULE:
            ...
            txtStatus = localized("membership-status-wrong-module",
                    ...
                    CommunityDictEnum.PRINTFLOWLITE_SUPPORT.getWord(getLocale()),
                    CommunityDictEnum.MEMBERSHIP.getWord(getLocale()));
            break;
        ...
        String htmlMembership = String.format(liFormat,
                ...
                Objects.toString(memberCard.getMemberOrganization(), "-"));
        ...
        helper.encloseLabel("button-import-membercard",
                ...
                hasEditorAccess);
        //
        final String urlWiki = CommunityDictEnum.PRINTFLOWLITE_WIKI_URL.getWord();
        helper.addAppendLabelAttr("PrintFlowLite-wiki-url", urlWiki,
                MarkupHelper.ATTR_HREF, urlWiki);

        final String urlCommunity =
                CommunityDictEnum.PRINTFLOWLITE_COMMUNITY_URL.getWord();
        helper.addAppendLabelAttr("PrintFlowLite-community-url", urlCommunity,
                MarkupHelper.ATTR_HREF, urlCommunity);

        final String urlHelpDesk =
                CommunityDictEnum.PRINTFLOWLITE_SUPPORT_URL.getWord();
        helper.addAppendLabelAttr("PrintFlowLite-helpdesk-url", urlHelpDesk,
                MarkupHelper.ATTR_HREF, urlHelpDesk);

        //
        labelWrk = new Label("PrintFlowLite-url",
        ...
        //
        labelWrk = new Label("PrintFlowLite-licenses-url",
                this.localized("license-info-link"));
        labelWrk.add(new AttributeModifier(MarkupHelper.ATTR_HREF,
                ServerUrlPath.DOCS_LICENSES_INDEX_HTML));
        add(labelWrk);

        //
        labelWrk = new Label("PrintFlowLite-source-code-url",
    ...
    /**
     * Adds PrintFlowLite specific environment (if present).
     *
     * @param helper
     *            Mark-up helper.
     */
    private void addEnvironment(final MarkupHelper helper) {

        final String widEnv = "environment";

        if (!ServerPropSystemEnv.isPresent()) {
            helper.discloseLabel(widEnv);
            return;
        }
        helper.addLabel(widEnv, NounEnum.ENVIRONMENT);

        final boolean isDocker = ServerPropSystemEnv.isDockerized();
        helper.encloseLabel("container-key", ServerPropSystemEnv.V_CONTAINER,
                isDocker);
        if (isDocker) {
            helper.addLabel("container", ServerPropSystemEnv.CONTAINER_DOCKER);
        }

        final Map<String, String> serverEnv = ServerPropSystemEnv.getEnv();

        //
        add(new PropertyListView<String>("env-entry",
                new ArrayList<>(serverEnv.keySet())) {
            ...
            protected void populateItem(final ListItem<String> item) {
                final String key = item.getModelObject();
                item.add(new Label("env-key", key));
                item.add(new Label("env-value", serverEnv.get(key)));
            }
        });
    }
