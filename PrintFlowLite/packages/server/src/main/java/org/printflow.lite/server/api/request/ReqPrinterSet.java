...
public final class ReqPrinterSet extends ApiRequestMixin {
    ...
    protected void onRequest(final String requestingUser, final User lockedUser)
        ...
        /*
         * INVARIANT: PPD file MUST exist.
         */
        if (StringUtils.isNotBlank(dto.getPpdFile())) {
            final File filePPD =
                    PROXY_PRINT_SERVICE.getPPDFile(dto.getPpdFile());
            if (!filePPD.exists()) {
                setApiResult(ApiResultCodeEnum.ERROR, "msg-file-not-present",
                        filePPD.getAbsolutePath());
                return;
            }
        }
        /*
         * INVARIANT: PDL transform file MUST exist.
         */
        if (StringUtils.isNotBlank(dto.getPdlTransformFile())) {
            final File filePPD = PROXY_PRINT_SERVICE
                    .getRawPrintTransformFile(dto.getPdlTransformFile());
            if (!filePPD.exists()) {
                setApiResult(ApiResultCodeEnum.ERROR, "msg-file-not-present",
                        filePPD.getAbsolutePath());
                return;
