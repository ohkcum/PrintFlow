...
import org.printflow.lite.server.helpers.account.UserAccountContextHtmlFactory;
import org.printflow.lite.server.pages.ACLOidAdminSummaryPanel;
import org.printflow.lite.server.pages.ACLOidSummaryPanel;
import org.printflow.lite.server.pages.ACLOidUserSummaryPanel;
import org.printflow.lite.server.pages.ACLRoleSummaryPanel;
...
public final class UsersPage extends AbstractAdminListPage {
    ...
    private final class UserListView extends PropertyListView<User> {
        ...
        /** */
        private final UserAccountContext accountCtxPrintFlowLite;
        ...
        /** */
        private static final String WID_EMAIL_PGP_ICON = "email-pgp-icon";
        ...
        UserListView(final String id, final List<User> list,
            ...
            this.accountCtxPrintFlowLite =
                    UserAccountContextFactory.getContextPrintFlowLite();
        ...
        protected void populateItem(final ListItem<User> item) {
            ...
            if (isErased) {
                ...
                final boolean hasPGP = USER_SERVICE.getUserAttrValue(user,
                        UserAttrEnum.PGP_PUBKEY_ID) != null;
                if (hasPGP) {
                    helper.addTransparentModifyAttr(WID_EMAIL_PGP_ICON,
                            MarkupHelper.ATTR_SRC,
                            MarkupHelper.IMG_PATH_USER_MAIL_OPENPGP);
                } else {
                    helper.discloseLabel(WID_EMAIL_PGP_ICON);
                }
                //
                labelWrk = new Label("userId");
                if (user.getInternal()) {
            ...
            final RegistrationStatusEnum userRegStatus =
                    USER_SERVICE.getUserRegistrationStatus(user, onDate);

            if (userRegStatus == RegistrationStatusEnum.PENDING) {
                color = MarkupHelper.CSS_TXT_WARN;
                signal = AdjectiveEnum.PENDING.uiText(getLocale())
                        .toLowerCase();
            } else if (userRegStatus == RegistrationStatusEnum.EXPIRED) {
                color = MarkupHelper.CSS_TXT_ERROR;
                signal = AdjectiveEnum.EXPIRED.uiText(getLocale())
                        .toLowerCase();
            } else if (isErased) {
                color = MarkupHelper.CSS_TXT_ERROR;
            ...
            item.add(new Label("balance-amount",
                    this.accountCtxPrintFlowLite.getFormattedUserBalance(user,
            ...
            if (this.accountCtxPaperCut == null || isErased) {
                ...
                helper.addTransparant("balance-PrintFlowLite-icon")
                        .add(new AttributeModifier(MarkupHelper.ATTR_SRC,
                                UserAccountContextHtmlFactory
                                        .getContext(
                                                UserAccountContextEnum.PrintFlowLite)
                                        .getImgUrl()));
            }
            ...
            MarkupHelper.modifyLabelAttr(labelWrk,
                    MarkupHelper.ATTR_DATA_PrintFlowLite, user.getUserId());
    ...
    private void handlePage() {
        ...
        filter.setRegistration(req.getSelect().getRegistration());
