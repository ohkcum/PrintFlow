 ...
 *
 */
@WebServlet(name = "LibreJsLicenseServlet", urlPatterns = { LibreJsLicenseServlet.SERVLET_URL_PATTERN })
public final class LibreJsLicenseServlet extends HttpServlet {
    ...
    /** */
    private static final Logger LOGGER = LoggerFactory.getLogger(LibreJsLicenseServlet.class);

    /** */
    private static final SSLContext SSLCONTEXT_TRUST_SELFSIGNED = InetUtils.createSslContextTrustSelfSigned();
    /**
     * .
     */
     ...
     */
    private static final String PATH_BASE = ServerUrlPath.LIBREJS;
    ...
    protected void doGet(final HttpServletRequest req,
        ...
        final LibreJsLicenseEnum libreLicense = unwrapLibreJsURL(reqURL, unwrappedURL);
        ...
        final byte[] licenceStart = LibreJsHelper.getJsLicenseStartTag(libreLicense).getBytes();
        ...
        if (this.isWrappedWicketJs(req)) {
            ...
            this.writeFromLocalHost(req, resp, urlUnwrapped.getPath(),
     ...
     *                     If IO error.
     */
    private void writeFromLocalHost(final HttpServletRequest req,
            final HttpServletResponse resp,
        ...
        if (WebServer.isSSLRedirect()) {
            ...
            localhostURL = new URL(InetUtils.URL_PROTOCOL_HTTPS, InetUtils.LOCAL_HOST,
                    Integer.parseInt(
                            ConfigManager.getServerSslPortLocal()),
                    unwrappedPath);
            ...
            localhostURL = new URL(InetUtils.URL_PROTOCOL_HTTP, InetUtils.LOCAL_HOST,
                    req.getLocalPort(),
                    unwrappedPath);
        }
        ...
        if (iPathBase > 0) {
            ...
            if (iLicense > 0) {
                ...
                if (iUnwrappedPath > 0) {
                    ...
                    final String license = wrappedURL.substring(iLicense, iUnwrappedPath);
