...
import org.printflow.lite.server.pages.UserPasswordReset;
import org.printflow.lite.server.pages.UserPinReset;
...
public final class PageUser extends AbstractAdminPage {
    ...
    private static final String CSS_CLASS_NUMBERS_ONLY_UUID =
            "26aa37e8-6166-11f0-b513-ff453bc913a9";

    /** */
    private void handlePasswordPanels() {

        final Integer maxPasswordLength =
                UserPasswordReset.getMaxPasswordLength();
        this.add(PasswordPanel.createPopulate("user-user-pw-panel",
                "user-user-pw", maxPasswordLength));
        this.add(PasswordPanel.createPopulate("user-user-pw-confirm-panel",
                "user-user-pw-confirm", maxPasswordLength));

        final Map<String, String> attrMap = new HashedMap<>();
        attrMap.put(MarkupHelper.ATTR_CLASS, CSS_CLASS_NUMBERS_ONLY_UUID);

        final Integer maxPinLength = UserPinReset.getMaxPinLength();
        this.add(PasswordPanel.createPopulate("user-user-pin-panel",
                "user-user-pin", maxPinLength, attrMap));
        this.add(PasswordPanel.createPopulate("user-user-pin-confirm-panel",
                "user-user-pin-confirm", maxPinLength, attrMap));
    }
    ...
    public PageUser(final PageParameters parameters) {
        ...
        helper.addButton("button-user-pin-reset", HtmlButtonEnum.RESET);
        helper.addButton("button-user-pin-erase", HtmlButtonEnum.ERASE);
        ...
        helper.addLabel("button-pin",
                getLocalizer().getString("user-pin", this));
