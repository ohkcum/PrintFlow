...
import org.printflow.lite.core.system.SystemFileDescriptorCount;
...
import org.printflow.lite.server.pages.JobTicketQueueInfoPanel;
...
import org.printflow.lite.server.pages.StatsFinancialDetailsPanel;
import org.printflow.lite.server.pages.StatsPageTotalPanel;
import org.printflow.lite.server.pages.StatsPrintInTotalPanel;
...
public final class SystemStatusPanel extends Panel {
    ...
    /** */
    private static final String WID_PANEL_FILTERED_REQUESTS =
            "filtered-requests-panel";
    ...
    public void populate(final boolean hasEditorAccess) {
        ...
        if (ServerPropSystemEnv.isDockerized()) {
            helper.addLabel("container-prompt", NounEnum.ENVIRONMENT);
            helper.addLabel("container-name", StringUtils.capitalize(
                    ServerPropSystemEnv.CONTAINER_DOCKER.toLowerCase()));
        } else {
            helper.discloseLabel("container-prompt");
        }
        ...
        final boolean isDoSFilter = cm.isConfigValue(Key.SYS_DOSFILTER_ENABLE);
        final boolean isInetAccessFilter =
                cm.isConfigValue(Key.SYS_INETFILTER_ENABLE);
        final boolean isHTTP2RateControl = ConfigManager.isHTTP2Enabled();

        if (isDoSFilter || isInetAccessFilter || isHTTP2RateControl) {

            final FilteredRequestsPanel filteredRequestsPanel =
                    new FilteredRequestsPanel(WID_PANEL_FILTERED_REQUESTS,
                            isDoSFilter, isInetAccessFilter,
                            isHTTP2RateControl);

            add(filteredRequestsPanel);
            filteredRequestsPanel.populate();
        } else {
            helper.discloseLabel(WID_PANEL_FILTERED_REQUESTS);
        }
        ...
        Label labelNews = new Label("PrintFlowLite-news", html);
    ...
    /**
     * Retrieves the news as HTML from local cache or from the PrintFlowLite site
     * when cache expired.
     *
        ...
        final CircuitBreakerOperation operation =
                new CircuitBreakerOperation() {
                    ...
                    private HttpGet getNewsRequest() {
                        ...
                        url.append(CommunityDictEnum.PRINTFLOWLITE_WWW_DOT_ORG_URL
                                .getWord());
