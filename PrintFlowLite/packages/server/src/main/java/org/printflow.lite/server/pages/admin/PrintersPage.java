...
import org.printflow.lite.core.dao.helpers.AbstractPagerReq;
...
import org.printflow.lite.server.helpers.HtmlPrinterImgEnum;
...
public final class PrintersPage extends AbstractAdminListPage {
    ...
    /** */
    private static final String CSS_CLASS_PRINTER_RAWPRINT_PPD_DOWNLOAD =
            "sp-printer-rawprint-ppd-download";

    /** */
    private static final String CSS_CLASS_PRINTER_RAWPRINT_TRANSFORM_DOWNLOAD =
            "sp-printer-rawprint-transform-download";

    /** */
    private static final String DOWNLOAD_INDICATOR = " ⇩ ";
    ...
    */
    private class PrintersListView extends PropertyListView<Printer> {
        ...
        protected void populateItem(final ListItem<Printer> item) {
            ...
            final boolean isPrinterPresent = cupsPrinter != null;
            ...
            final StringBuilder cupsClassMembership = new StringBuilder();
            ...
            if (!isPrinterPresent) {
                ...
                for (Iterator<String> iterator =
                        cupsPrinter.getPrinterClasses().iterator(); iterator
                                .hasNext();) {
                    if (cupsClassMembership.length() > 0) {
                        cupsClassMembership.append(", ");
                    }
                    cupsClassMembership.append(iterator.next());
                }
            }
            ...
            item.add(createVisibleLabel(cupsClassMembership.length() > 0,
                    "cups-printer-class-memberships",
                    String.format("(%s)", cupsClassMembership.toString())));
            ...
            this.populateItemLocalFileDownload(item, attrLookup,
                    PrinterAttrEnum.CUSTOM_PPD_EXT_FILE, "ppd-ext-file",
                    CSS_CLASS_PRINTER_PPDE_DOWNLOAD,
                    localized("title-download-ppde"));

            this.populateItemLocalFileDownload(item, attrLookup,
                    PrinterAttrEnum.RAW_PRINT_PPD_FILE, "rawprint-ppd-file",
                    CSS_CLASS_PRINTER_RAWPRINT_PPD_DOWNLOAD,
                    localized("title-download-rawprint-ppd"));

            this.populateItemLocalFileDownload(item, attrLookup,
                    PrinterAttrEnum.RAW_PRINT_TRANSFORM_FILE,
                    "rawprint-transform-file",
                    CSS_CLASS_PRINTER_RAWPRINT_TRANSFORM_DOWNLOAD,
                    localized("title-download-rawprint-transform"));
            ...
            if ((snmpDate == null && snmpDto == null) || !isPrinterPresent) {
            ...
            final boolean isDisabled = printer.getDisabled();
            ...
            item.add(createVisibleLabel(
                    isPrinterPresent && !isDisabled && !isConfigured
                            && !isInternal,
            ...
            item.add(createVisibleLabel(
                    isPrinterPresent && !isDisabled && !isConfigured
                            && isInternal,
                    "printer-needs-configuration-internal",
                    localized("printer-needs-configuration")));
            ...
            final Label labelPrinterName;

            if (cupsPrinter == null) {
                labelPrinterName = new Label("printerName");
                ...
                labelPrinterName = new Label("printerName",
                        printer.getPrinterName().concat(DOWNLOAD_INDICATOR));

                MarkupHelper.modifyLabelAttr(labelPrinterName,
                        MarkupHelper.ATTR_DATA_PrintFlowLite,
                ...
                if (StringUtils.isBlank(nameWlk)) {
                    ...
                    if (cupsPrinter.isPpdPresent()) {
                        nameWlk = nameWlk.concat(DOWNLOAD_INDICATOR);
                    }
                }

                labelWrk = helper.addLabel("driver",
                        Objects.toString(nameWlk, "-"));
            ...
            final boolean signalDisconnected =
                    !PROXY_PRINT_SERVICE.isConnectedToCups();
            final Component compWrk = helper.addTransparantDisabled(
                    "signal-icon-disconnected", signalDisconnected);
            if (signalDisconnected) {
                compWrk.add(new AttributeModifier(MarkupHelper.ATTR_SRC,
                        MarkupHelper.IMG_PATH_GENERIC_ERROR))
                        .add(new AttributeModifier(MarkupHelper.ATTR_TITLE,
                                localized("signal-disconnected")));
            }
            ...
            /*
             * Set the uid in 'data-PrintFlowLite' attribute, so it can be picked up
        ...
        /**
         * Adds download link for printer related files.
         *
         * @param item
         * @param attrLookup
         * @param printerAttr
         ...
         * @param title
         */
        void populateItemLocalFileDownload(final ListItem<Printer> item,
                final PrinterAttrLookup attrLookup, PrinterAttrEnum printerAttr,
                final String widLabel, final String cssClass,
                final String title) {

            final Printer printer = item.getModelObject();

            final String fileBaseName = attrLookup.get(printerAttr);

            final boolean isBaseName = StringUtils.isNotBlank(fileBaseName);
            final boolean doesFileExist;
            ...
            if (isBaseName) {
                ...
                switch (printerAttr) {
                case CUSTOM_PPD_EXT_FILE:
                    file = PROXY_PRINT_SERVICE.getPPDExtFile(fileBaseName);
                    break;
                case RAW_PRINT_PPD_FILE:
                    file = PROXY_PRINT_SERVICE.getRawPrintPPDFile(fileBaseName);
                    break;
                case RAW_PRINT_TRANSFORM_FILE:
                    file = PROXY_PRINT_SERVICE
                            .getRawPrintTransformFile(fileBaseName);
                    break;

                default:
                    throw new SpException(
                            printerAttr.getDbName().concat(" NOT supported."));
                }
            ...
            final String fileBaseNameDisplay;
            if (doesFileExist) {
                fileBaseNameDisplay = fileBaseName.concat(DOWNLOAD_INDICATOR);
            } else {
                fileBaseNameDisplay = fileBaseName;
            }

            final Label labelWrk = createVisibleLabel(isBaseName, widLabel,
                    fileBaseNameDisplay);
