...
/*! PrintFlowLite jQuery Mobile User Web App | (c) 2020 Datraverse B.V. | GNU
...
/*
 * PrintFlowLite jQuery Mobile User Web App
 */
(function($, window, document, JSON, _ns) {
    ...
    function PageAccountTrx(_i18n, _view, _model, _api) {
        ...
        $('#page-accounttrx').on('pagecreate', function(event) {
            ...
            $(this).on('click', '.sp-btn-accounttrx-report', null, function() {
                _panel.v2m();
                _api.download("report-user", _panel.input, "AccountTrxList", $(this).attr('data-PrintFlowLite'));
                return true;
            });
            ...
            $(this).on('click', ".sp-download-receipt", null, function() {
                _api.download("pos-receipt-download-user", null, $(this).attr('data-PrintFlowLite'));
                return false;
            }).on('click', ".sp-download-invoice", null, function() {
                _api.download("pos-invoice-download-user", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });
        ...
        $('#page-doclog').on('pagecreate', function(event) {
            ...
            $(this).on('click', '.sp-doclog-accounttrx-info', null, function() {
                var html = _view.getPageHtml('DocLogAccountTrxAddin', {
                    docLogId: $(this).attr('data-PrintFlowLite')
                }) || 'error';
                ...
                _onInboxRestorePrintIn($(this).attr('data-PrintFlowLite'), false);
                return false;
            }).on('click', '.sp-inbox-restore-printin-replace', null, function() {
                _onInboxRestorePrintIn($(this).attr('data-PrintFlowLite'), true);
                return false;
            });
    ...
    function PagePdfProp(_i18n, _view, _model) {
        ...
        _setVisibility = function() {
            var links = _view.isCbChecked($('#pdf-uri-links')),
                encrypt = _view.isCbChecked($('#pdf-encryption')),
                sign = _view.isCbChecked($('#pdf-pgp-signature'));

            _view.visible($('#sp-pdf-uri-link-detail'), links);
        ...
        _clearPasswords = function() {
            var wlk = _model.propPdf.pw;
            // Do NOT prefill password fields with saved values.
            _ns.PasswordPanel.clear($('#pdf-pw-owner'));//.val(wlk.owner);
            _ns.PasswordPanel.clear($('#pdf-pw-owner-c'));//.val(wlk.owner);
            _ns.PasswordPanel.clear($('#pdf-pw-user'));//.val(wlk.user);
            _ns.PasswordPanel.clear($('#pdf-pw-user-c'));//.val(wlk.user);
        }
        /*
         * Model to View
         */
        _m2V = function() {
            ...
            wlk = _model.propPdf.links;
            _view.setSelectedValue($('#pdf-uri-links-width'), wlk.width);
            _view.setSelectedValue($('#pdf-uri-links-color'), wlk.color);
        ...
        _v2M = function() {
            ...
            wlk = _model.propPdf.links;
            wlk.width = $('#pdf-uri-links-width').val();
            wlk.color = $('#pdf-uri-links-color').val();
    ...
    function PageOutbox(_i18n, _view, _model, _api) {
        var _this = this,
            ...
            _onAccountTrxInfo = function(src, jobticket) {
                var html = _view.getPageHtml('OutboxAccountTrxAddin', {
                    jobFileName: src.attr('data-PrintFlowLite'),
                    jobticket: jobticket
                }) || 'error';
        ...
        $('#page-outbox').on("pagecreate", function(event) {
            ...
            $(this).on('click', '.sp-outbox-cancel-job', null, function() {
                _this.onOutboxDeleteJob($(this).attr('data-PrintFlowLite'), false);
                if (_model.user.stats.outbox.jobCount > 0) {
                    return _refresh();
                }
                return _close();
            }).on('click', '.sp-outbox-cancel-jobticket', null, function() {
                _this.onOutboxDeleteJob($(this).attr('data-PrintFlowLite'), true);
                if (_model.user.stats.outbox.jobCount > 0) {
                    return _refresh();
                }
                return _close();
            }).on('click', '.sp-outbox-preview-job', null, function() {
                _api.download("pdf-outbox", null, $(this).attr('data-PrintFlowLite'));
                return false;
            }).on('click', '.sp-outbox-preview-jobticket', null, function() {
                _api.download("pdf-jobticket", null, $(this).attr('data-PrintFlowLite'));
                return false;
    ...
    function PageSend(_i18n, _view, _model) {
        ...
        $("#page-send").on("pagecreate", function(event) {
            ...
            $('#button-send-send').click(function() {
                var PdfOutDto = JSON.stringify({
                    removeGraphics: _model.removeGraphics,
                    ecoprint: _model.ecoprint,
                    grayscale: _model.pdfGrayscale,
                    rasterize: _model.pdfRasterize,
                    links: _model.pdfLinks
                });
                _this.onSend($('#send-mailto').val(), _model.pdfPageRanges, PdfOutDto);
                return false;
            });
    ...
    function PageTOTPUser(_i18n, _view, _model, _api) {
        ...
        var _pageId = '#page-user-totp',
            ...
            //
            _m2v = function() {
                var isEnabled = _view.isCbChecked($('#sp-user-totp-enable-chb')),
                    isTelegram = _view.isCbChecked($('#sp-user-totp-telegram-enable-chb'));
        ...
        $(_pageId).on('pagecreate', function(event) {
            ...
            $('#user-totp-apply-btn').click(function() {
                var res = _api.call({
                    'request': 'user-totp-set',
                    dto: JSON.stringify({
                        'enabled': _view.isCbChecked($('#sp-user-totp-enable-chb')),
                        'totp': $('#sp-user-totp-enable-verify-code').val()
                    })
                });
                _view.showApiMsg(res);
                if (res.result.code === '0') {
                    $('#user-totp-cancel-btn').click();
                }
                return false;
            });
            ...
            $(this).on('change', "input:checkbox[id='sp-user-totp-enable-chb']", null, function(e) {
                ...
                _view.visible($('#sp-user-totp-enable-verify-code-div'), isEnabled);
            });
     ...
     * Constructor
     */
    function PagePdfValidate(_i18n, _view, _model) {
        /*
         * Page is pre-loaded, so no _class needed
         */
        var _page = new _ns.Page(_i18n, _view, '#page-pdfvalidate'),
            _DROPZONE,
            _DROPZONE_HTML,
            _self = _ns.derive(_page),
            //
            _showValidateButton = function(enable) {
                _view.visible($('#sp-btn-pdfvalidate-submit').parent(), enable);
            },
            _showResetButton = function(enable) {
                _view.visible($('#sp-btn-pdfvalidate-reset').parent(), enable);
            },
            _setUploadFeedbackDefault = function(dropzone) {
                var feedback = $('#sp-pdfvalidate-upload-feedback');
                feedback.removeClass('sp-txt-warn').removeClass('sp-txt-valid').addClass('sp-txt-info');
                if (dropzone) {
                    feedback.html(_DROPZONE_HTML);
                } else {
            ...
            _onUploadStart = function() {
                $('#sp-btn-pdfvalidate-reset').click();
            },
            _onUploadDone = function() {
                _showValidateButton(false);
                _showResetButton(true);
            },
            _onUploadMsgWarn = function(warn, files, filesStatus) {
                $('#sp-pdfvalidate-upload-feedback').html(_ns.DropZone.getHtmlWarning(_i18n, warn, files, filesStatus)).show();
                _showValidateButton(false);
                _showResetButton(true);
                $('#sp-pdfvalidate-upload-file').val('');
            },
            _onUploadMsgInfo = function(files, info) {
                $('#sp-pdfvalidate-upload-feedback').addClass('sp-txt-valid').html(info);
                _showResetButton(true);
            };
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            $(this).on('input', '#sp-pdfvalidate-upload-file', function() {
                var show = $(this).val();
                _showValidateButton(show);
                _showResetButton(show);
            });

            $('#sp-btn-pdfvalidate-reset').click(function() {
                _showValidateButton(false);
                _showResetButton(false);
                _setUploadFeedbackDefault(_DROPZONE);
                return true;
            ...
            $('#sp-btn-pdfvalidate-submit').click(function() {
                return true;
            });

            $('#sp-pdfvalidate-upload-form').submit(function(e) {
                var files = document.getElementById('sp-pdfvalidate-upload-file').files;
                e.preventDefault();
                _ns.DropZone.sendFiles(files, _model.pdfValidateUploadUrl, _model.pdfValidateUploadFileParm,
                    null, function() {
                        return null;
                    }, _model.pdfValidateMaxBytes, _model.pdfValidateFileExt, _i18n,
                    function() {// before send
                        _onUploadStart();
                    }, function() {// after send
                        _onUploadDone();
            ...
            if (_DROPZONE) {
                zone = $('#sp-pdfvalidate-upload-feedback');
                zone.addClass('sp-dropzone-small');

                _ns.DropZone.setCallbacks(zone, 'sp-dropzone-hover-small', //
                    _model.pdfValidateUploadUrl, _model.pdfValidateUploadFileParm,
                    null, function() {
                        return null;
                    }, _model.pdfValidateMaxBytes, _model.pdfValidateFileExt, _i18n,
                    function() {// before send
            ...
            $('#sp-pdfvalidate-upload-form').on('change', '#sp-pdfvalidate-upload-file', function() {
                var show = $(this).val();
                _showValidateButton(show);
                _showResetButton(show);
                _setUploadFeedbackDefault(_DROPZONE);
            ...
            $(this).on('input', '#sp-pdfvalidate-upload-file', function() {
                ...
                if (_ns.logger.isDebugEnabled()) {
                    (function logFiles(show, files) {
                        ...
                        _ns.logger.debug('onInput #sp-pdfvalidate-upload-file: show = '
                            + (show ? 'true' : 'false') + ' | ' + msg);
                    })(show, document.getElementById('sp-webprint-upload-file').files);
                }
    ...
    function PageMain(_i18n, _view, _model, _api) {
        ...
        this.setThumbnails = function() {
            ...
            $.each(_model.myJobPages, function(key, page) {
                ...
                if (tnUrl) {
                    ...
                    item += '<img class="' + _ns.CSS_CLASS_THUMBNAIL_PNG + ' ' + _ns.CSS_CLASS_PRELOAD_SPINNER + '" onload="org.printflow.lite.removeImgHeight(this)" ';
        ...
        _onPageInfoTap = function(sel) {
            ...
            if (iImg > -1) {
                ...
                //
                html = _view.getUserPageHtml('PdfDocumentPropsAddIn', {
                    ijob: _model.iPopUpJob
                }) || 'error';
                $('#sp-popup-job-info-pdf-props').html(html);
                //
        ...
        $('#page-main').on('pagecreate', function(event) {
            ...
            $('#button-mini-pdfvalidate').click(function() {
                var pageId = '#page-pdfvalidate';
                /*
                 * This page is a fixed part of WebAppUserPage.html
                 */
                _view.changePage(pageId);
    ...
    function PagePrintSettings(_i18n, _view, _model) {
        var _this = this,
            ...
            CUSTOM_HTML5_DATA_ATTR = 'data-PrintFlowLite',
            CUSTOM_HTML5_DATA_ATTR_ICON = 'data-PrintFlowLite-icon',
            CSS_CLASS_SELECT_IPP_OPTION = '_PFL_select_ipp_option',
            ...
            _isMediaSourceAutoSelected = function() {
                return $("select[data-PrintFlowLite='media-source']").val() === 'auto';
            },
            _isPageRotate180Selected = function() {
                return $("select[data-PrintFlowLite='org.printflow.lite.int-page-rotate180']").val() === '1';
            },
            _showNumberUpPreview = function() {
                var selLandscape = $('#cb-nup-preview-landscape');
                if (selLandscape.length > 0) {
                    _ns.NumberUpPreview.show(_view, $("select[data-PrintFlowLite='number-up']").val(), _isPageRotate180Selected(), $("select[data-PrintFlowLite='org.printflow.lite-finishings-punch']").val(), $("select[data-PrintFlowLite='org.printflow.lite-finishings-staple']").val(), _view.isCbChecked(selLandscape));
                }
            },
            ...
            _probeChangePageRotate180 = function(target) {
                if (target.attr(CUSTOM_HTML5_DATA_ATTR) !== 'org.printflow.lite.int-page-rotate180') {
                    return;
                }
            ...
            _probeChangeStaple = function(target) {
                if (target.attr(CUSTOM_HTML5_DATA_ATTR) !== 'org.printflow.lite-finishings-staple') {
                    return;
                }
                _showNumberUpPreview();
            },
            _probeChangePunch = function(target) {
                if (target.attr(CUSTOM_HTML5_DATA_ATTR) !== 'org.printflow.lite-finishings-punch') {
                    return;
                }
                _showNumberUpPreview();
            },
            ...
            _onChangeMediaSource = function(target) {
                ...
                // Note: _model.myPrinter might not be present when media-source change is triggered.
                if (isMediaClash && _model.myPrinter && !_model.myPrinter.fitPrintScaling) {
                    _view.checkRadioValue('print-page-scaling-enum', _model.PRINT_SCALING_ENUM.NONE);
                }
            ...
            _m2v = function() {
                ...
                _onChangeMediaSource($("select[data-PrintFlowLite='media-source']"));

                // resolve visibility
                _onChangeMediaSource($("select[data-PrintFlowLite='media-type']"));
            },
            //
        ...
        $('#page-printer-settings').on('pagecreate', function(event) {
            ...
            _view.visibleCheckboxRadio($('#print-page-scaling-enum-fit'), _model.myPrinter.fitPrintScaling);
            _view.checkRadioValue('print-page-scaling-enum', _model.PRINT_SCALING_ENUM.NONE);
            ...
            selMediaSource = $("select[data-PrintFlowLite='media-source']");
    ...
    function PagePrint(_i18n, _view, _model, _api, _ctrl) {
        ...
        var _this = this,
            ...
            _onQuickPrinterSearch = function(target, filter, startPosition, paging) {
                ...
                if (res.result.code === '0') {
                    ...
                    $.each(_quickPrinterCache, function(key, item) {
                        html += "<li class=\"ui-mini ui-li-has-icon\" data-icon=\"false\" data-PrintFlowLite=\"" + key + "\">";
            ...
            _onPrint = function(isClose, calcCostMode) {
                ...
                selWlk = $("#print-collate");
                printCollate = selWlk.length > 0 ? _view.isCbChecked(selWlk) : true;

                return _this.onPrint(clearScope, isClose, _view.isCbChecked($("#print-remove-graphics"))
                    , _view.isCbChecked($("#print-ecoprint")), printCollate
            ...
            _onPrintAsync = function() {
                var selMsg = $(".sp-proxy-print-wait");
                selMsg.show();
                selMsg.addClass('sp-spinner-small');
                _ns.Utils.asyncFoo(function() {
                    _onPrint(true);
                    selMsg.removeClass('sp-spinner-small');
                    selMsg.hide();
                });
            },
        ...
        $('#page-print').on('pagecreate', function(event) {
            ...
            $("#sp-print-quicksearch-printer-div .sp-quicksearch-buttons .ui-btn").click(function() {
                _onQuickPrinterSearch(filterablePrinter, $("#sp-print-qs-printer").val(), $(this).attr('data-PrintFlowLite'), true);
            });
    ...
    function Model(_i18n) {
        ...
        this.pdfValidateEnabled = false;
        this.pdfValidateDropZoneEnabled = false;
        this.pdfValidateMaxBytes = 0;
        this.pdfValidateFileExt = [];
        this.pdfValidateUploadUrl = null;
        this.pdfValidateUploadFileParm = null;
        this.pdfValidateUploadFontParm = null;
        ...
        this.getMediaSourceMedia = function(source) {
            ...
            if (_this.myPrinter) {
                // org.printflow.lite.core.dto.IppMediaSourceMappingDto
    ...
    function Controller(_i18n, _model, _view, _api, _cometd, _userEvent, _deviceEvent, _proxyprintEvent) {
        var _util = _ns.Utils,
            ...
            _savePdfLinks = function(sel) {
                _model.pdfLinks = _view.isCbChecked($(sel));
            },
        ...
        _initUser = function(loginRes) {
             ...
             * org.printflow.lite.server.cometd.BayeuxAuthenticator
             */
        ...
        this.init = function() {
            ...
            /** Mantis: #1315 */
            if (res.watchdogMessageEnable) {
                fooWakeUp = _this.onWakeUp;
            } else {
                fooWakeUp = _this.onWakeUpAutoRestore;
            }

            _ns.configAppWatchdog(fooWakeUp, res.watchdogIntervalSecs, res.watchdogAllowedDelaySecs, _this.onWakeUpAutoRestore);
            ...
            // pdfValidate
            _model.pdfValidateEnabled = true;
            _model.pdfValidateDropZoneEnabled = true;
            _model.pdfValidateMaxBytes = res.pdfValidateMaxBytes;
            _model.pdfValidateFileExt = res.pdfValidateFileExt;
            _model.pdfValidateUploadUrl = res.pdfValidateUploadUrl;
            _model.pdfValidateUploadFileParm = res.pdfValidateUploadFileParm;
            _model.pdfValidateUploadFontParm = null;
         ...
         * Callbacks: page send
         */
        _view.pages.send.onSend = function(mailto, ranges, dto) {
            var res;
        ...
        _view.pages.pdfprop.onDownload = function() {
            var PdfOutDto,
                pageRanges = $('#pdf-page-ranges').val(),
                filters;
            ...
            window.location.assign(_api.getUrl4Pdf(pageRanges, _model.pdfJobIndex, PdfOutDto));
            _model.myShowUserStatsGet = true;
            return true;
        };
    ...
    $.PrintFlowLite = function() {
        ...
        _view.pages = {
            ...
            pdfValidate: new PagePdfValidate(_i18n, _view, _model),
            userPinReset: new _ns.PageUserPinReset(_i18n, _view, _model),
        ...
        this.init = function() {
            ...
            } else if (window.location.pathname === '/payment' || window.location.pathname.startsWith('/payment/sp-oauth')) {
    ...
    // Instantiate the singleton PrintFlowLite instance when the DOM is leaded.
    $(function() {
        $.PrintFlowLite = new $.PrintFlowLite();
...
}(jQuery, this, this.document, JSON, this.org.printflow.lite));
