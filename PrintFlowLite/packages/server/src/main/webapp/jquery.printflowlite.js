...
/*! PrintFlowLite jQuery Mobile Common | © 2020 Datraverse B.V. | GNU Affero
 ...
 */
function _popupMessage(msg, top, width) {
    $("<div class='ui-overlay-shadow ui-body-a ui-corner-all' style='max-width:600px'>" + "<center><p style='padding: 10px;'>" + msg + "<p></center></div>").css({
        "z-index": 5000,
        display: "block",
        position: "relative",
        /* opacity: 0.96, */
...
function _copyHtmlToClipboard(elementID) {
    ...
    $temp.remove();
    _popupMessage("📄 &rarr; 📋", $(window).scrollTop(), "200px");
};

function _evaluateKeypressNumber(e) {
    if (String.fromCharCode(e.keyCode).match(/[^0-9]/g)) {
        return false;
    }
};

function _elementIdToJQueryFormat(name) {
...
(function($, window, document, _ns) {
    ...
    var _watchdogIntervalSecs,
        _watchdogAllowedDelaySecs,
        _watchdogTimer,
     ...
     * Constructor
     */
    _passwordPanel = function() {

        const _classIconEyeOpen = 'ui-icon-sp-password-eye-open',
            _classIconEyeClose = 'ui-icon-sp-password-eye-close',
            _attrBtn2InputId = 'data-PrintFlowLite-key';

        var _getPanel = function(selInput) {
            return selInput.closest('.sp-password-panel');
        };
        var _getButton = function(selInput) {
            return _getPanel(selInput).find('button');
        };
        var _getInput = function(selButton) {
            return $("#" + _elementIdToJQueryFormat(selButton.attr(_attrBtn2InputId)));
        };

        // -----------------------------
        this.onWebAppInit = function() {
            /* misc */
            $(document).on('click', '.sp-btn-secret-visibility', null, function() {
                var input = _getInput($(this));
                if (input.attr('type') === 'password') {
                    input.attr('type', 'text');
                    $(this).removeClass(_classIconEyeOpen);
                    $(this).addClass(_classIconEyeClose);
                } else {
                    input.attr('type', 'password');
                    $(this).removeClass(_classIconEyeClose);
                    $(this).addClass(_classIconEyeOpen);
                }
                $(this).enhanceWithin();
                input.focus();
                return false;
        ...
        // -----------------------------
        this.clear = function(selInput) {
            var btn = _getButton(selInput);

            btn.removeClass(_classIconEyeOpen);
            btn.removeClass(_classIconEyeClose);
            btn.addClass(_classIconEyeOpen);

            selInput.val('').attr('type', 'password');
            return selInput;
        };
        // -----------------------------
        this.visible = function(selInput, show) {
            if (show) {
                _getPanel(selInput).show();
            } else {
                _getPanel(selInput).hide();
            }
        };

    };

    _ns.PasswordPanel = new _passwordPanel();
    ...
    _ns.commonWebAppInit = function() {
        ...
        /* misc */
        _ns.PasswordPanel.onWebAppInit();
    };
    ...
    _onWatchdogHeartbeat = function() {
        ...
        wokeUp = (delta > ((_watchdogIntervalSecs + _watchdogAllowedDelaySecs) * 1000));
    ...
    _ns.checkAppWakeUpAutoRestore = function() {
        if (_onAppWakeUpAutoRestore) {
            var now = new Date().getTime(),
                delta = (now - _lastAppHeartbeat);
            _lastAppHeartbeat = now;
            if (delta > ((_watchdogIntervalSecs + _watchdogAllowedDelaySecs) * 1000)) {
    ...
    /** Mantis: #1315 */
    _ns.configAppWatchdog = function(onAppWakeUp, watchdogIntervalSecs, watchdogAllowedDelaySecs, onAppWakeUpAutoRestore) {
        _onAppWakeUp = onAppWakeUp;
        _watchdogIntervalSecs = watchdogIntervalSecs;
        _watchdogAllowedDelaySecs = watchdogAllowedDelaySecs;
    ...
    _ns.startAppWatchdog = function(defer) {
        ...
        if (!_watchdogTimer) {
            _watchdogTimer = window.setInterval(_onWatchdogHeartbeat, _watchdogIntervalSecs * 1000);
        }
    };
    ...
    _ns.Cometd = function() {
        ...
        _self.configure = function(maxNetworkDelay) {
            ...
            /*
             * IMPORTANT: Because websocket is not supported yet by PrintFlowLite
             * server,
            ...
            $.cometd.addListener('/meta/handshake', function(handshake) {
                ...
                // handshake.ext.'org.printflow.lite.authn';
    ...
    _ns.PanelAccountTrxBase = {
        ...
        input: {
            ...
            select: {
                user_id: null,
                account_id: null,
                // See Java: org.printflow.lite.core.dao.enums.AccountTrxTypeEnum
                trxType: null,
                date_from: null,
                date_to: null,
                text: null
    ...
    _ns.PanelPaperCutAccountTrxBase = {
        ...
        input: {
            ...
            select: {
                user_id: null,
                // org.printflow.lite.ext.papercut.PapercutAccountTrx.TrxType
                trxType: null,
                date_from: null,
                date_to: null,
                text: null
            ...
            sort: {
                // org.printflow.lite.ext.papercut.PaperCutDb.Field
                field: 'TRX_DATE',
                ascending: false
            }
        },
    ...
    _ns.PanelDocLogBase = {
        ...
        m2v: function() {
            ...
            if (this.input.select.job_state) {
        ...
        onDocStoreDownloadDelete: function(jqParent, _api, _view) {
            ...
            jqParent.on('click', '.sp-doclog-docstore-archive-download', null, function() {
                _api.download("pdf-docstore-archive", null, $(this).attr('data-PrintFlowLite'));
                return false;
            }).on('click', '.sp-doclog-docstore-journal-download', null, function() {
                _api.download("pdf-docstore-journal", null, $(this).attr('data-PrintFlowLite'));
                return false;
            }).on('click', '.sp-doclog-docstore-delete', null, function() {
                $('#sp-doclog-store-delete-popup .sp-ui-btn-ok').attr('data-PrintFlowLite',
                    $(this).attr('data-PrintFlowLite'));
    ...
    _ns.QuickSearchUtils = {
        ...
        setNavigation: function(_view, qsButtons, rsp) {
            ...
            if (qsButtons) {
                ...
                selWlk.attr('data-PrintFlowLite', rsp.prevPosition).show();

                selWlk = qsButtons.find('.sp-button-curr');
                selWlk.attr('data-PrintFlowLite', rsp.currPosition).show();
                selWlk.html(rsp.currPage);

                selWlk = qsButtons.find('.sp-button-next');
                selWlk.attr('data-PrintFlowLite', rsp.nextPosition).show();

                selWlk = qsButtons.find('.sp-button-last');
                selWlk.attr('data-PrintFlowLite', rsp.lastPosition).show();
                ...
                if (filter && filter.length > 0) {
                    ...
                    if (res.result.code === _ns.ApiResultCodeEnum.OK) {
                        ...
                        $.each(_quickObjectCache, function(key, item) {
                            html += "<li class=\"ui-mini\" data-icon=\"false\" data-PrintFlowLite=\"" + key + "\">";
     ...
     * Constructor
     */
    _ns.PageUserPinReset = function(_i18n, _view) {
        var _page,
            _self,
            _onSelectReset;
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            _view.asyncFocus($('#user-pin-reset'));
    ...
    _ns.PageUserPasswordReset = function(_i18n, _view) {
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            _view.asyncFocus($('#user-pw-reset'));
        }).on('pagebeforehide', function(event, ui) {
            var selPw = $("#user-pw-reset"),
                selPwCf = $("#user-pw-reset-confirm");
            selPw.val('');
            selPwCf.val('');
            _ns.PasswordPanel.clear(selPw);
            _ns.PasswordPanel.clear(selPwCf);
        });
        /*
         * IMPORTANT
         */
    ...
    _ns.PageLogin = function(_i18n, _view, _api) {
        ...
        _onAuthModeSelect = function(modeSelected) {
            ...
            if ($('.sp-btn-login-mode-oauth').length === 0) {
                if (nMethods < 2) {
                    $('#sp-login-modes').hide();
                }
            } else if (nMethods < 1) {
                $('#sp-login-modes-header').hide();
            }
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            $('#sp-btn-registration').click(function() {
                var res = _api.call({
                    request: "user-registration-permit"
                });
                if (res.result.code === _ns.ApiResultCodeEnum.OK) {
                    $('#page-login-content').hide();
                    $('#page-registration-content').show();
                } else {
                    _view.showApiMsg(res);
                }
                return false;
            ...
            $('#sp-btn-registration-cancel').click(function() {
                $('#page-login-content').show();
                $('#page-registration-content').hide();
                return false;
            });

            $('#sp-btn-registration-send').click(function() {
                var res = _api.call({
                    request: "user-registration",
                    dto: JSON.stringify({
                        userName: $('#sp-registration-user-name').val(),
                        fullName: $('#sp-registration-user-fullname').val(),
                        email: $('#sp-registration-email').val(),
                        password: $('#sp-registration-user-password').val(),
                        passwordConfirm: $('#sp-registration-user-password-confirm').val()
                    })
                });

                if (res.result.code === _ns.ApiResultCodeEnum.OK) {
                    _view.resetForm($('#sp-registration-user-form'))
                    $('#sp-btn-registration-cancel').click();
                    _view.showApiMsg(res);
                } else {
                    _view.showApiMsg(res);
                }
            ...
            $('.sp-btn-login-mode-oauth').click(function() {
                var res = _api.call({
                    request: "oauth-url",
                    dto: JSON.stringify({
                        provider: $(this).attr('data-PrintFlowLite'),
                        instanceId: $(this).attr('data-PrintFlowLite-type')
                    })
                });
    ...
    _ns.View = function(_i18n, _api) {
        ...
        this.jqPlotMonthDayFormat = function() {
            return $.jsDate.regional[$.jsDate.config.defaultLocale].PrintFlowLiteDayMonthFormat;
        };
         ...
         * Resets a <form>.
         */
        this.resetForm = function(jqFormSel) {
            // Get javascript object and execute reset(). 
            jqFormSel[0].reset();
        };
        ...
        this.setButtonText = function(sel, val) {
            sel.text(val);
        };
        ...
        this.visibleInput = function(jqsel, show) {
            this.visible(jqsel.closest('.ui-input-text'), show);
        };
        ...
        /**
         */
        this.disableUserInput = function(disable) {
            var jqsel = $('#sp-overlay-block-user-input');
            if (disable) {
                $('button').attr("disabled", true);
                $('input').attr("disabled", true);
                $('a').hide(); // no easy way to disable, so hide it
                jqsel.css({
                    display: 'block',
                    position: 'absolute',
                    top: 0,
...
}(jQuery, this, this.document, this.org.printflow.lite));
...
}(this, this.document, this.navigator, this.org.printflow.lite));
...
(function($, window, document, _ns) {
    ...
    _ns.NumberUpPreview.show = function(_view, numberUp, rotate180, punch, staple, landscape) {
        var clazz,
            sel = $('.sp-nup-preview-sheet').filter('[data-PrintFlowLite=' + numberUp + '-' + (landscape ? 'l' : 'p') + ']'),
