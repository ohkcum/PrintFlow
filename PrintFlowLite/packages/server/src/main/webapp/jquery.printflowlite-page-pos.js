...
/*! PrintFlowLite jQuery Mobile Admin POS Page | (c) 2020 Datraverse B.V. | GNU
    ...
    _ns.PagePointOfSale = function(_i18n, _view, _model, _api, isMain,
                ...
                if (filter && filter.length > 0) {
                    ...
                    if (res.result.code === _ns.ApiResultCodeEnum.OK) {
                        ...
                        $.each(res.dto.items, function(key, item) {
                            ...
                            html += "<div>";
                            html += "<a tabindex=\"0\" href=\"#\" data-PrintFlowLite=\"" + item.key + "\" class=\"sp-download-receipt ui-icon-arrow-d " + btnCls + "\">PDF</a>";
                            if (item.userEmail) {
                                html += "<a tabindex=\"0\" href=\"#\" data-PrintFlowLite=\"" + item.key + "\" class=\"sp-download-mail ui-icon-mail " + btnCls + "\">" + item.userEmail + "</a>";
                            }
                            html += "</div>";
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            $(this).on('click', ".sp-download-receipt", null, function() {
                _api.download("pos-receipt-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', ".sp-download-mail", null, function() {
                var res = _api.call({
                    ...
                    // PrimaryKeyDto
                    dto: JSON.stringify({
                        key: $(this).attr('data-PrintFlowLite')
                    })
                });
                _view.showApiMsg(res);
                return false;
