...
/*! PrintFlowLite jQuery Mobile Job Tickets Page | (c) 2020 Datraverse B.V. | GNU
...
(function($, window, document, JSON, _ns) {
                ...
                if (filter && filter.length > 0) {
                    ...
                    if (res.result.code === '0') {
                        ...
                        $.each(_quickCache, function(key, item) {
                            html += "<li class=\"ui-mini\" data-icon=\"false\" data-PrintFlowLite=\"" + key + "\"><a tabindex=\"0\" href=\"#\">" + item.text + "</a></li>";
                        });
                    } else {
    ...
    _ns.PageJobTickets = function(_i18n, _view, _model, _api) {
        ...
        var _page = new _ns.Page(_i18n, _view, "#page-jobtickets", "PageJobTickets"),
            ...
            _onDocLogPrintOutReversePopup = function(docLogId, positionTo) {
            ...
            _onSaveJob = function(jobFileName) {
                ...
                $('#sp-jobticket-popup').find('select').each(function() {
                    ippOptions[$(this).attr('data-PrintFlowLite')] = $(this).find(':selected').val();
                });
            ...
            _onSaveJobCopies = function(jobFileName) {
                ...
                $('.sp-doclog-accounttrx-info-table').find('._sp-edit-copies').each(function() {
                    var name = $(this).closest('tr').find('._sp-edit-copies-account').attr('data-PrintFlowLite');
                    accounts[$(this).attr('data-PrintFlowLite-key')] = {
                        copies: $(this).val(),
                        accountName: name
                    };
                });
            ...
            _setJobCopiesButtons = function() {
                ...
                selTable.find('._sp-edit-copies').each(function() {
                    if ($(this).val() !== $(this).attr('data-PrintFlowLite')) {
                        enable = true;
                    }
                });
            ...
            _onProcessAll = function(mode) {
                ...
                window.setTimeout(function() {
                    tickets.each(function() {
                        ...
                        if (mode === _MODE_PRINT) {
                            res = _execJob($(this).attr('data-PrintFlowLite'), true);
                        } else if (mode === _MODE_CLOSE) {
                            res = _closeJob($(this).attr('data-PrintFlowLite'));
                        } else {
                            res = _cancelJob($(this).attr('data-PrintFlowLite'));
                        }
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            $(this).on('click', '.sp-outbox-cancel-jobticket', null, function() {
                _onCancelPopup($(this).attr('data-PrintFlowLite'), $(this));
                ...
                _onPrintPopup($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-jobticket-print-close', null, function() {
                var res = _closeJob($(this).attr('data-PrintFlowLite'));
                if (res.result.code === "0") {
                    _view.showApiMsg(res);
                    _refresh();
                } else {
                ...
                _onPrintCancel($(this).attr('data-PrintFlowLite'));

            }).on('click', '.sp-jobticket-print-retry', null, function() {
                _onPrintRetry($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-jobticket-settle', null, function() {
                _onPrintPopup($(this).attr('data-PrintFlowLite'), $(this), true);

            }).on('click', '.sp-jobticket-edit', null, function() {
                _onEditPopup($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-jobticket-settings', null, function() {
                _onSettingsPopup($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-doclog-accounttrx-refund', null, function() {
                _onDocLogAccountTrxRefundPopup($(this).attr('data-PrintFlowLite'), $(this));
            }).on('click', '.sp-doclog-printout-reverse', null, function() {
                _onDocLogPrintOutReversePopup($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-doclog-accounttrx-info', null, function() {
                _onDocLogAccountTrxPopup($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-doclog-ticket-reopen', null, function() {
                var res = _onDocLogTicketReopen($(this).attr('data-PrintFlowLite'));
                if (res.result.code === "0") {
                    _view.showApiMsg(res);
                ...
                _onJobTicketAccountTrxPopup($(this).attr('data-PrintFlowLite'), $(this));

            }).on('click', '.sp-doclog-accounttrx-info-table ._sp-edit-apply', null, function() {
                _onSaveJobCopies($(this).attr('data-PrintFlowLite'));
                return false;
                ...
                _view.visible($(this).closest('tr').find('._sp-edit-cost'), $(this).val() === $(this).attr('data-PrintFlowLite'));
                ...
                _onExecJob($(this).attr('data-PrintFlowLite'), true);

            }).on('click', "#sp-jobticket-popup-btn-print-retry", null, function() {
                _onExecJob($(this).attr('data-PrintFlowLite'), true, true);

            }).on('click', "#sp-jobticket-popup-btn-settle", null, function() {
                _onExecJob($(this).attr('data-PrintFlowLite'), false);
                ...
                _onSaveJob($(this).attr('data-PrintFlowLite'));

            }).on('click', '#sp-jobticket-cancel-popup-btn-yes', null, function() {
                var res = _cancelJob($(this).attr('data-PrintFlowLite'), $('#sp-jobticket-cancel-popup-reason').val());
                if (res.result.code === "0") {
                    $('#sp-jobticket-popup').popup('close');
                    _refresh();
                }
            ...
            }).on('click', '#sp-doclog-accounttrx-refund-btn-yes, #sp-doclog-printout-reverse-btn-yes', null, function() {
                var res = _refundAccountTrx($(this).attr('data-PrintFlowLite'));
                $('#sp-jobticket-popup').popup('close');
                if (res.result.code === "0") {
                    _pnlDocLog.refresh();
                }
