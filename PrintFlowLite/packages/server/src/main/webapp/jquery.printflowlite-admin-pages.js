...
/*! PrintFlowLite jQuery Mobile Admin Pages | (c) 2020 Datraverse B.V. | GNU
...
/*
 * PrintFlowLite jQuery Mobile Admin Pages
 */
(function($, window, document, JSON, _ns) {
    ...
    _ns.PageUser = function(_i18n, _view, _model) {
        ...
        var _page = new _ns.Page(_i18n, _view, '#page-user', 'admin/PageUser'),
            ...
            //
            _m2v = function() {
                // See Java org.printflow.lite.dto.UserDto
                var emailOther,
                ...
                _view.visible($('#button-user-pw-erase'),
                    _model.editUser.internal && _model.editUser.dbId && _model.editUser.internalPw);
                _view.visible($('#button-user-pin-erase'),
                    _model.editUser.dbId && _model.editUser.internalPIN);
            },
            //
            _v2m = function() {
                ...
                if (!_model.editUser.dbId) {
                    ...
                    if (!_view.checkPwMatch($('#user-user-pin'), $('#user-user-pin-confirm'), true)) {
                        return false;
                    }
                    _model.editUser.pin = $('#user-user-pin').val();
                }
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            $(this).on('click', '#button-user-pin-dialog', null, function() {
                _view.showPageAsync('#page-user-pin-reset', 'UserPinReset', function() {
                    $('#user-pin-reset-title').html(_model.editUser.userName);
                });
                return false;
            });
            $(this).on('click', '#button-user-pin-erase', null, function() {
                _self.onEraseUserPIN();
                return false;
            });
    ...
    _ns.PageConfigProp = function(_i18n, _view, _model) {
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            var val = _model.editConfigProp.value,
                ...
                selPassword = $('#config-prop-value-password'),
                multi = $('#config-prop-value-multiline');

            single.val('');
            _view.visibleInput(single, false);

            _ns.PasswordPanel.clear(selPassword);
            _ns.PasswordPanel.visible(selPassword, false);

            multi.val('').hide();

            _view.setButtonText($('#button-save-configprop'),
                _model.editConfigProp.saveButtonText);
            ...
            window.setTimeout(function() {
                ...
                if (_model.editConfigProp.multiline) {
                    multi.val(val).show();
                    selFocus = multi;
                } else if (_model.editConfigProp.password) {
                    _ns.PasswordPanel.visible(selPassword, true);
                    selPassword.val(val);
                    selFocus = selPassword;
                } else {
                    _view.visibleInput(single, true);
                    single.val(val);
                    selFocus = single;
                }
                selFocus.focus();
    ...
    _ns.PageDevice = function(_i18n, _view, _model, _api) {
        ...
        _m2v = function() {
            ...
            attrFullString.forEach(function(name) {
                // escape ALL the points '.'
                var id = _elementIdToJQueryFormat(name);
                $('#' + id).val(device.attr[name]);
            });
    ...
    _ns.PageQueue = function(_i18n, _view, _model, _api) {
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            $(this).on('change', "input:checkbox[id='queue-trusted']", null, function(e) {
                var isTrusted = $(this).is(':checked');
                _view.visibleCheckboxRadio($('#queue-basic-auth'), _model.editQueue.basicAuthApplicable && !isTrusted);
            });
            ...
            _view.checkCb('#queue-basic-auth', _model.editQueue.basicAuth);
            ...
            _view.visibleCheckboxRadio($('#queue-basic-auth'),
                _model.editQueue.basicAuthApplicable && !_view.isCbChecked($('#queue-trusted')));
    ...
    _ns.PagePrinter = function(_i18n, _view, _model) {
        ...
        var _page = new _ns.Page(_i18n, _view, '#page-printer', 'admin/PagePrinter'),
            _self = _ns.derive(_page),
            _onChangeJobTicket, _onChangeChargeType, _onBrowsePPDE,
            _onChangePPDE, _showAllMediaRows;
        ...
        $(_self.id()).on('pagecreate', function(event) {
            ...
            var htmlWlk, accounting, mediaSource, ppdFile, ppdExtFile, pdlTransformFile, data;
            ...
            ppdFile = $('#printer-ppd-file');
            ppdExtFile = $('#printer-ppd-ext-file');
            pdlTransformFile = $('#printer-pdl-transform-file');
            data = {};
            ...
            // check raw printer class
            _view.visible($('.printer-raw-enabled'), _model.editPrinter.rawPrintingEnabled);
            ...
            // PPD file
            htmlWlk = _model.editPrinter.rawPrintingEnabled ? _view.getAdminPageHtml('PPDFileOptionsAddin', data) : "";
            ppdFile.html(htmlWlk).enhanceWithin();
            _view.setSelectedValue(ppdFile, _model.editPrinter.ppdFile);

            // PDL transform file
            htmlWlk = _model.editPrinter.rawPrintingEnabled ? _view.getAdminPageHtml('PDLTransformFileOptionsAddin', data) : "";
            pdlTransformFile.html(htmlWlk).enhanceWithin();
            _view.setSelectedValue(pdlTransformFile, _model.editPrinter.pdlTransformFile);
    ...
    _ns.PageAdmin = function(_i18n, _view, _model, _api) {
        ...
        _reloadOptionsPanel = function() {
            $('.PrintFlowLite-detail[name=Options]').click();
        };
        ...
        _refreshPanelByUrl = function(url, wClass, skipBeforeLoad) {
             ...
             * is the full url:
             *
             * '/wicket/bookmarkable/org.printflow.lite.server.pages.'
             * +
             * wClass
             */
            $.mobile.loading("show");
         ...
         *  
         */
        _printoutJobStateSet = function(doclogID, ippJobState, selPopup) {
            var res = _api.call({
                request: 'printout-jobstate-set',
                dto: JSON.stringify({
                    docLogId: doclogID,
                    jobState: ippJobState
                })
            });
            selPopup.popup('close');
            if (res.result.code === "0") {
                $('#button-doclog-apply').click();
            }
        ...
        $(_self.id()).on('pagebeforeshow', function(event, ui) {
             ...
             * Generic loading
             */
            $('.PrintFlowLite-detail').click(function() {
                _loadPanel($(this).attr('name'));
                return false;
            });

            $(this).on('click', '.PrintFlowLite-detail-prev', null, function(ev) {
            ...
            $(this).on('click', '#sp-btn-gdpr-download', null, function() {
                ...
                _self.onGdprDownload($(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', '.sp-edit-shared-account', null, function() {
                _self.onEditSharedAccount($(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', '.sp-account-transaction', null, function() {
                ...
                pnl.applyDefaults();
                pnl.input.select.account_id = $(this).attr('data-PrintFlowLite');
                // skipBeforeLoad
                pnl.refresh(true);
                return false;
            });
            ...
            $(this).on('click', '.sp-btn-accounts-report', null, function() {
                var pnl = _panel.AccountsBase;
                pnl.v2m();
                _self.onDownload("report", pnl.input, "AccountList", $(this).attr('data-PrintFlowLite'));
                return true;
            });
            ...
            $(this).on('click', '.sp-btn-accounttrx-report', null, function() {
                var pnl = _panel.AccountTrxBase;
                pnl.v2m();
                _self.onDownload("report", pnl.input, "AccountTrxList", $(this).attr('data-PrintFlowLite'));
                return true;
            });

            $(this).on('click', ".sp-download-receipt", null, function() {
                _self.onDownload("pos-receipt-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            ...
            $(this).on('click', '.sp-edit-config', null, function() {
                _self.onEditConfigProp($(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', '.sp-edit-device', null, function() {
                _self.onEditDevice($(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            //------------------------
            $(this).on('click', '.sp-printout-jobstate-to-canceled', null, function() {
                var sel = $('#sp-printout-popup-jobstate-to-canceled');
                sel.attr('data-PrintFlowLite', $(this).attr('data-PrintFlowLite'));
                sel.popup('open', { positionTo: $(this) });

            }).on('click', '#sp-printout-popup-jobstate-to-canceled .sp-ui-btn-ok', null, function() {
                _printoutJobStateSet($('#sp-printout-popup-jobstate-to-canceled').attr('data-PrintFlowLite'),
                    'IPP_JOB_CANCELED', $('#sp-printout-popup-jobstate-to-canceled'));

            }).on('click', '#sp-printout-popup-jobstate-to-canceled .sp-ui-btn-cancel', null, function() {
                $('#sp-printout-popup-jobstate-to-canceled').popup('close');
            });

            //------------------------
            $(this).on('click', '.sp-printout-jobstate-to-completed', null, function() {
                var sel = $('#sp-printout-popup-jobstate-to-completed');
                sel.attr('data-PrintFlowLite', $(this).attr('data-PrintFlowLite'));
                sel.popup('open', { positionTo: $(this) });

            }).on('click', '#sp-printout-popup-jobstate-to-completed .sp-ui-btn-ok', null, function() {
                _printoutJobStateSet($('#sp-printout-popup-jobstate-to-completed').attr('data-PrintFlowLite'),
                    'IPP_JOB_COMPLETED', $('#sp-printout-popup-jobstate-to-completed'));

            }).on('click', '#sp-printout-popup-jobstate-to-completed .sp-ui-btn-cancel', null, function() {
                $('#sp-printout-popup-jobstate-to-completed').popup('close');
            });
            ...
            $(this).on('click', '.sp-edit-user', null, function() {
                _self.onEditUser($(this).attr('data-PrintFlowLite'));
                return false;
            });

            $(this).on('click', '.sp-user-gdpr', null, function() {
                _self.onGdprUser($(this).attr('data-PrintFlowLite'), $(this));
                return false;
            });
            ...
            $(this).on('click', '.sp-user-log', null, function() {
                var pnl = _panel.DocLogBase;
                pnl.applyDefaults();
                pnl.input.select.user_id = $(this).attr('data-PrintFlowLite');
                // skipBeforeLoad
                pnl.refresh(true);
                return false;
            });
            ...
            $(this).on('click', '.sp-btn-users-report', null, function() {
                var pnl = _panel.UsersBase;
                pnl.v2m();
                _self.onDownload("report", pnl.input, "UserList", $(this).attr('data-PrintFlowLite'));
                return true;
            });
            ...
            $(this).on('click', '.sp-edit-user-group', null, function() {
                _self.onEditUserGroup($(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', '.sp-user-group-users', null, function() {
                var pnl = _panel.UsersBase;
                pnl.applyDefaults();
                pnl.input.select.usergroup_id = $(this).attr('data-PrintFlowLite');
                // skipBeforeLoad
                pnl.refresh(true);
                return false;
            });
            ...
            $(this).on('click', '.sp-user-group-account', null, function() {
                var pnl = _panel.AccountsBase;
                pnl.applyDefaults();
                pnl.input.select.name_text = $(this).attr('data-PrintFlowLite');
                pnl.input.select.accountType = $(this).attr('data-PrintFlowLite-type');
                // skipBeforeLoad
                pnl.refresh(true);
                return false;
            });
            ...
            $(this).on('click', '.sp-btn-user-groups-report', null, function() {
                var pnl = _panel.UserGroupsBase;
                pnl.v2m();
                _self.onDownload("report", pnl.input, "UserGroupsList", $(this).attr('data-PrintFlowLite'));
                return true;
            });
            ...
            $(this).on('click', '.sp-edit-queue', null, function() {
                _self.onEditQueue($(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', '.sp-queue-log', null, function() {
                ...
                pnl.applyDefaults();
                pnl.input.select.queue_id = $(this).attr('data-PrintFlowLite');
                // skipBeforeLoad
                pnl.refresh(true);
                return false;
            });
            ...
            $(this).on('click', '.sp-edit-printer', null, function() {
                _self.onEditPrinter($(this).attr('data-PrintFlowLite'));
                return false;
            });

            $(this).on('click', '.sp-printer-snmp-refresh', null, function() {
                _self.onPrinterSnmp($(this).attr('data-PrintFlowLite'));
                return false;
            });

            $(this).on('dblclick', '.sp-printer-opt-download', null, function() {
                _self.onDownload("printer-opt-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });

            $(this).on('dblclick', '.sp-printer-ppd-download', null, function() {
                _self.onDownload("printer-ppd-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });

            $(this).on('dblclick', '.sp-printer-ppde-download', null, function() {
                _self.onDownload("printer-ppde-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });
            $(this).on('dblclick', '.sp-printer-rawprint-ppd-download', null, function() {
                _self.onDownload("printer-rawprint-ppd-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });
            $(this).on('dblclick', '.sp-printer-rawprint-transform-download', null, function() {
                _self.onDownload("printer-rawprint-transform-download", null, $(this).attr('data-PrintFlowLite'));
                return false;
            });
            ...
            $(this).on('click', '.sp-printer-log', null, function() {
                ...
                pnl.applyDefaults();
                pnl.input.select.printer_id = $(this).attr('data-PrintFlowLite');
                // skipBeforeLoad
                pnl.refresh(true);
                return false;
            });
            ...
            $(this).on('click', '.sp-btn-reports-user-printout-tot', null, function() {
                ...
                pnl.v2m();
                _self.onDownload("report", pnl.input, "UserPrintOutTotals", $(this).attr('data-PrintFlowLite'));
                return true;
            });
            ...
            $(this).on('click', '#apply-auth', null, function() {
                if (_self.onApplyAuth(_view.getRadioValue('auth.method'))) {
                    _reloadOptionsPanel();
                }
                return false;
            });
            ...
            $(this).on('change', "input:checkbox[id='internal-users.registration.enable']", null, function(e) {
                _panel.Options.onInternalUsersRegistrationEnabled($(this).is(':checked'));
            });
            ...
            $(this).on('change', "input:checkbox[id='flipswitch-payment-plugin-generic-online']", null, function(e) {
                _self.onPaymentGatewayOnline($(this).is(':checked'));
            });
            ...
            $(this).on('click', '#sp-btn-sys-mode-apply', null, function() {
                ...
                _self.onApplySysModeChange($(this).attr('data-PrintFlowLite'));
            });
            ...
            $(this).on('click', "#sp-btn-atomfeed-refresh", null, function() {
                _self.onAtomFeedRefresh();
                return false;
            });
            ...
            $(this).on('click', '#cliapp-admin-passkey-reset', null, function() {
                _self.onCliappAdminPasskeyReset();
                return false;
            });
