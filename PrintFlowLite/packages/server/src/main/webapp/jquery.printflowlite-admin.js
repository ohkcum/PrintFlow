...
/*! PrintFlowLite jQuery Mobile Admin Web App | © 2020 Datraverse B.V. | GNU
...
/*
 * PrintFlowLite jQuery Mobile Admin Web App
 */
(function($, window, document, JSON, _ns) {
    ...
    _ns.Controller = function(_i18n, _model, _view, _api, _cometd) {
        ...
        _fillConfigPropsYN = function(props, propNames) {
            propNames.forEach(function(name) {
                props[name] = _view.isCheckedYN($('#' + _elementIdToJQueryFormat(name)));
            });
            return props;
        };
        ...
        _fillConfigPropsText = function(props, propNames) {
            propNames.forEach(function(name) {
                props[name] = $('#' + _elementIdToJQueryFormat(name)).val();
            });
            return props;
        };

        /** */
        _clearConfigPropsText = function(propNames) {
            propNames.forEach(function(name) {
                $('#' + _elementIdToJQueryFormat(name)).val('');
            });
        };
        ...
        _handleLoginResult = function(authMode, data) {
            ...
            if (_model.user.loggedIn) {
                 ...
                 * See: Java org.printflow.lite.server.cometd.BayeuxAuthenticator
                 */
         ...
         *
         */
        _saveConfigProps = function(props, denyEmptySecrets) {
            var res = _api.call({
                request: 'config-set-props',
                dto: JSON.stringify(props),
                denyEmptySecrets: denyEmptySecrets
            });
            _view.showApiMsg(res);
            return (res.result.code === '0');
        };
        ...
        _view.pages.admin.onApplyAuth = function(method) {
            ...
            ret = _saveConfigProps(props, denyEmptySecrets);
            if (ret) {
            _clearConfigPropsText(['auth.ldap.admin-password']);
            }
            return ret;
        };
        ...
        _view.pages.admin.onApplySmtp = function(security) {
            ...
            _saveConfigProps(props, denyEmptySecrets);
            _clearConfigPropsText(['mail.smtp.password']);
        };
        ...
        _view.pages.admin.onApplyInternalUsers = function(enabled) {
            var props = {};
            _fillConfigPropsYN(props, ['internal-users.enable',
                'internal-users.registration.enable',
                'internal-users.user-can-change-password']);
            _fillConfigPropsText(props, [
                'internal-users.registration.email.domain-whitelist'
            ]);
            return _saveConfigProps(props);
        };
        ...
        _view.pages.admin.onApplyPaperCut = function(enable, enableDb) {
            ...
            _saveConfigProps(props, denyEmptySecrets);
            _clearConfigPropsText(['papercut.webservices.auth-token', 'papercut.db.password']);
        };
        ...
        _view.pages.admin.onApplyProxyPrint = function() {
            ...
            _fillConfigPropsYN(props, ['proxy-print.non-secure', 'proxy-print.delegate.enable', 'proxy-print.repair.enable', 'webapp.user.proxy-print.clear-inbox.enable', 'webapp.user.proxy-print.clear-inbox.prompt', 'proxy-print.fast-inherit-printin-ipp.enable']);
        ...
        _view.pages.admin.onApplyImap = function(enabled, security) {
            ...
            _saveConfigProps(props, denyEmptySecrets);
            _clearConfigPropsText(['print.imap.password']);
        };
        ...
        _view.pages.admin.onPaymentGatewayOnline = function(online) {
            _view.showApiMsg(_api.call({
        ...
        _view.pages.admin.onAtomFeedRefresh = function() {
            _view.showApiMsg(_api.call({
                request: 'atomfeed-refresh'
            }));
        };
        ...
        _view.pages.admin.onGdprUser = function(uid, positionTo) {
            $('#sp-btn-gdpr-download').attr('data-PrintFlowLite', uid);
         ...
         *
         */
        _view.pages.user.onEraseUserPIN = function() {
            var res = _api.call({
                request: 'erase-user-pin',
                dto: JSON.stringify({
                    userDbId: _model.editUser.dbId
                })
            });
        ...
        _view.pages.configProp.onSave = function() {
            var prop = {},
                ok,
                sel = (_model.editConfigProp.multiline ? '#config-prop-value-multiline' :
                    (_model.editConfigProp.password ? '#config-prop-value-password' : '#config-prop-value'));
        ...
        _view.pages.queue.onSaveQueue = function() {
            ...
            _model.editQueue.basicAuth = $('#queue-basic-auth').is(':checked');
        ...
        _view.pages.printer.onSavePrinter = function() {
            ...
            _model.editPrinter.ppdFile = $('#printer-ppd-file').val();
            _model.editPrinter.ppdExtFile = $('#printer-ppd-ext-file').val();
            _model.editPrinter.pdlTransformFile = $('#printer-pdl-transform-file').val();
        ...
        _view.pages.printer.onSavePrinterMediaSources = function() {
            var res,
                ...
                dto = {}, // org.printflow.lite.dto.ProxyPrinterMediaSourcesDto
            ...
            $(".sp-printer-media-source-row").each(function() {
                ...
                // push: org.printflow.lite.core.dto.IppMediaSourceDto
                dto.sources.push({
                    active: active,
        ...
        _view.pages.printer.onSavePrinterCost = function() {
            var res,
                // org.printflow.lite.dto.ProxyPrinterCostDto
                dto = {};
        ...
        _view.pages.admin.onCliappAdminPasskeyReset = function() {
            var configKey = 'cliapp.auth.admin-passkey',
                selNew = $('#' + _elementIdToJQueryFormat(configKey)),
                selConfirm = $('#cliapp-auth-admin-passkey-confirm');
            if (selNew.val() !== selConfirm.val()) {
                _view.message(_i18n.format('msg-input-mismatch'));
            } else {
                var props = {};
                _fillConfigPropsText(props, [configKey]);
                _saveConfigProps(props);
                selNew.val('');
                selConfirm.val('');
            }
     ...
     *
     */
    $.PrintFlowLiteAdm = function(name) {
    ...
    $(function() {
        $.PrintFlowLiteAdm = new $.PrintFlowLiteAdm();
